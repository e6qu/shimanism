package aws_sns

import (
	"encoding/xml"
	"errors"
	"net/http"

	"github.com/e6qu/shimanism/internal/pubsub/domain"
)

type xmlErrorResponse struct {
	XMLName   xml.Name `xml:"ErrorResponse"`
	Xmlns     string   `xml:"xmlns,attr"`
	Error     xmlError `xml:"Error"`
	RequestId string   `xml:"RequestId"`
}

type xmlError struct {
	Type    string `xml:"Type"`
	Code    string `xml:"Code"`
	Message string `xml:"Message"`
}

func writeError(w http.ResponseWriter, status int, kind, code, message string) {
	w.Header().Set("Content-Type", "text/xml")
	w.WriteHeader(status)
	resp := xmlErrorResponse{
		Xmlns: snsNamespace,
		Error: xmlError{Type: kind, Code: code, Message: message},
	}
	out, _ := xml.MarshalIndent(resp, "", "  ")
	_, _ = w.Write([]byte(xml.Header))
	_, _ = w.Write(out)
}

func mapDomainError(w http.ResponseWriter, err error) {
	var de *domain.Error
	if !errorsAs(err, &de) {
		writeError(w, http.StatusInternalServerError, "Receiver", "InternalError", err.Error())
		return
	}
	switch de.Kind {
	case domain.KindNoSuchTopic:
		writeError(w, http.StatusNotFound, "Sender", "NotFound", de.Error())
	case domain.KindTopicAlreadyExists:
		writeError(w, http.StatusBadRequest, "Sender", "TopicAlreadyExists", de.Error())
	case domain.KindNoSuchSubscription:
		writeError(w, http.StatusNotFound, "Sender", "NotFound", de.Error())
	case domain.KindSubscriptionAlreadyExists:
		writeError(w, http.StatusBadRequest, "Sender", "SubscriptionAlreadyExists", de.Error())
	case domain.KindInvalidArgument:
		writeError(w, http.StatusBadRequest, "Sender", "InvalidParameter", de.Error())
	default:
		writeError(w, http.StatusInternalServerError, "Receiver", "InternalError", de.Error())
	}
}

// errorsAs is a small wrapper so the rest of the file doesn't carry
// the stdlib import twice.
func errorsAs(err error, target interface{}) bool { return errors.As(err, target) }
