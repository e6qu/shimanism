...
func mapDomainError(w http.ResponseWriter, err error) {
	...
	case domain.KindNoSuchInstance:
		writeError(w, http.StatusNotFound, "Sender", "CacheClusterNotFound", de.Error())
	case domain.KindInstanceAlreadyExists:
		writeError(w, http.StatusBadRequest, "Sender", "CacheClusterAlreadyExists", de.Error())
	case domain.KindInstanceNotAvailable:
		writeError(w, http.StatusBadRequest, "Sender", "InvalidCacheClusterState", de.Error())
