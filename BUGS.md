...
|---|---|---|---|---|
| 59 | P2 | kms/sockerless-lane (dead test) | 19.D PR2 | `services/kms/conformance/...` was never added to `scripts/run-sockerless-storage.sh`'s test list, so the AWS KMS through-shim sockerless lane added in 19.C (`TestSockerless_AWSKMS_Through_Shim`) **never executed in CI** — a silently-dead conformance test. Fixed by adding the kms package to the runner; the env it needs (`SOCKERLESS_AWS_ENDPOINT`, `SOCKERLESS_AZURE_KV_URL`, `SOCKERLESS_AZURE_TLS_PORT`) was already exported. Same commit wires the new Azure KV-keys + AWS-tagged-key Terraform lanes. |
| 58 | P2 | kms/gcp-frontend (fakes) | 19.D PR1 | GCP Cloud KMS frontend treated `keyRings.create`/`get` as **synthetic success** — `getKeyRing` returned 200 for any ring name, even one never created (real Cloud KMS 404s). A fake-behavior defect (class-of-bug rule "No fakes"). Fixed by promoting keyRing to a first-class, non-optional `domain.KMS` capability (`CreateKeyRing`/`GetKeyRing`): inmem (test backend-of-record) + GCP (native Cloud KMS) track rings honestly; AWS/Azure return `ErrNotSupported` (no honest data-plane home — see INTERSECTION.md keyRing analysis). Unknown ring → real 404, re-create → 409, key in non-existent ring → 404; verified by `TestGCPSDK_KMS_KeyRingExistence`. The missing GCP Cloud KMS *simulator* in sockerless filed as [sockerless#419](https://github.com/e6qu/sockerless/issues/419). |
| 53 | P2 | kms/gcp-frontend | 19.D PR1 | GCP Cloud KMS frontend omitted the Cloud KMS CRC32C data-integrity fields, so `gcloud kms encrypt`/`decrypt` rejected structurally-valid responses with "corrupted in-transit." Fix: compute CRC32C (Castagnoli) over ciphertext/plaintext, verify request-side `plaintext_crc32c`/`ciphertext_crc32c`/`additional_authenticated_data_crc32c` (mismatch → `INVALID_ARGUMENT`), emit `ciphertextCrc32c`/`plaintextCrc32c` + `verified*Crc32c`; decode request `bytes` tolerant of std/URL-safe base64; reject non-empty AAD (out of intersection). Also added `cryptoKeyVersions` list/get/`:destroy` (real key metadata; `:destroy` → `ScheduleKeyDeletion`) for `google_kms_crypto_key` TF support. |
| 56 | P2 | compute/azure-tf-conformance | 16.C PR7 | `TestSockerless_AzureCompute_Through_Shim_Terraform_Apply` in `sockerless_test.go`: uses `StartComputeServerAzureVMWithConfig` with passthrough → sockerless for Microsoft.Network + resource groups; `azurerm_linux_virtual_machine` create/destroy through the shim's azure_compute frontend. DNS `azure_terraform_test.go` BUG-44 stub also cleaned up (real test already existed in DNS `sockerless_test.go`). |
| 57 | P2 | compute/azure-cli-conformance | 16.C PR6 | `azure_compute` frontend needed `Config` / `HandlerWithConfig` / `serveMetadata` / `passthroughOr404` (same pattern as `azure_dns`). Added in this PR. `TestAzureCLI_Compute_VMList` exercises `az vm list` through the shim's Azure Compute frontend with sockerless's Entra token. |
