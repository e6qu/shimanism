...
|---|---|---|---|---|
| 44 | P2 | dns/azure-terraform-conformance | this PR | `hashicorp/azurerm`'s single `endpoints { resource_manager = "..." }` had to satisfy both DNS-specific calls (shim's frontend) and generic ARM calls (resource group + subscription). Added **ARM passthrough mode** to `internal/dns/frontends/azure_dns`: `NewWithPassthrough(d, upstream http.Handler)` forwards unmatched ARM paths to a configured upstream while DNS paths stay local. Through-shim Terraform test `TestSockerless_AzureDNS_Through_Shim_Terraform_Apply` wires the proxy to sockerless's Azure ARM mock under TLS with explicit cert pinning (RootCAs pool, no InsecureSkipVerify) and `SSL_CERT_FILE` threading the combined system + sockerless + shim cert bundle into the Terraform subprocess. Linux-only via SSL_CERT_FILE platform limit. Unit tests in `passthrough_test.go` pin the DNS-vs-passthrough dispatch boundary. |
