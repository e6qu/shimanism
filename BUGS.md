...
| BUG-44 | P2 | dns/azure-terraform-conformance | `hashicorp/azurerm` | `azurerm_dns_zone` requires an `azurerm_resource_group`, and that resource talks to ARM resource-group + subscription endpoints. The shim's Azure DNS frontend handles only `Microsoft.Network/dnsZones[+privateDnsZones]`; non-DNS ARM paths fall through. `azurerm` supports only one `endpoints { resource_manager = "..." }`, so the shim needs an **ARM passthrough mode** — forward unmatched ARM paths to a configured upstream (sockerless ARM mock in tests, real ARM in prod). Sockerless has the RG + subscription coverage (resourcegroups.go + subscription.go); the gap is the shim's ARM-passthrough infrastructure. | **15.D** |
