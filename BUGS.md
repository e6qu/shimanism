...
| BUG-42 | P3 | dns/gcp-backend | sockerless `simulators/gcp/dns.go` | Sockerless's Cloud DNS sim implements zone CRUD + direct `POST/GET/DELETE /rrsets` but is missing the **Changes API** (`POST /managedZones/{zone}/changes` + `GET .../changes/{id}` + `PATCH /rrsets/{name}/{type}`) that the SDK and `hashicorp/google` use for atomic record updates. The shim's GCP backend routes `PutRecordSet` / `DeleteRecordSet` through `dns.Changes.Create`, so the through-shim sockerless test fails today. Filed [e6qu/sockerless#298](https://github.com/e6qu/sockerless/issues/298). `TestSockerless_GCPCloudDNS_Through_Shim_ZoneLifecycle` `t.Skip`'d pending the upstream fix. | **15.D** |
