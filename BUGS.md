...
**38 filed · 33 fixed · 4 open · 1 false positive.** Phase 14.B closed BUG-34 (sockerless PR #225 — SB ATOM XML admin) and BUG-36 (sockerless PR #231 — SB raw AMQP/TLS transport, lets the shim's Send/Receive lanes use `CustomEndpoint` + `TLSConfig` instead of WebSocket-dial code). Added the storage multipart matrix (AWS S3 + GCS + Azure Blob, 3/3), SB queue Send/Receive, and SB topic Publish/Receive lanes. BUG-37 + BUG-38 filed as latent cross-cloud CopyObject gaps (Azure Blob `StartCopyFromURL` + GCS `rewriteTo`/`copyTo`); not blocking today since no current lane exercises CopyObject. Tracked upstream as [sockerless#232](https://github.com/e6qu/sockerless/issues/232) + [sockerless#233](https://github.com/e6qu/sockerless/issues/233). BUG-35 (Container Apps pre-pull) is the remaining Phase-14.B residual.
...
| BUG-37 | P3 | storage/azureblob-backend | `azblob` `StartCopyFromURL` | Adding a CopyObject through-shim sockerless cell with an Azure Blob destination is blocked on sockerless implementing the Copy Blob REST operation. The shim's `services/storage/backends/azureblob.CopyObject` calls `StartCopyFromURL`, which sends a blob-level PUT with the `x-ms-copy-source` header; sockerless's `handleBlobDataPlane` ignores that header and silently creates a zero-byte destination blob instead of copying. Filed upstream as [sockerless#232](https://github.com/e6qu/sockerless/issues/232). No current shim lane exercises this — the storage through-shim cells today only do Put/Get lifecycle. | **14.E** |
| BUG-38 | P3 | storage/gcs-backend | `gcsstorage.CopierFrom(src).Run` | Adding a CopyObject through-shim sockerless cell with a GCS destination is blocked on sockerless implementing the GCS object-copy REST endpoints. The shim's `services/storage/backends/gcs.CopyObject` calls `dst.CopierFrom(src).Run(ctx)`, which targets `POST /storage/v1/b/{src}/o/{obj}/rewriteTo/b/{dst}/o/{obj}` (or the legacy `copyTo` variant); neither is registered in sockerless GCS — only the `:compose` action under the object prefix is wired. Filed upstream as [sockerless#233](https://github.com/e6qu/sockerless/issues/233). Same latent gap as BUG-37 — surfaces only when a CopyObject lane is added. | **14.E** |
...
| [e6qu/sockerless#232](https://github.com/e6qu/sockerless/issues/232) — Azure Blob: Copy Blob via `x-ms-copy-source` header not implemented | ◐ open. Tracks BUG-37. Blocks CopyObject through-shim cells with an Azure Blob destination. |
| [e6qu/sockerless#233](https://github.com/e6qu/sockerless/issues/233) — GCS: `rewriteTo` / `copyTo` object-copy REST endpoints not implemented | ◐ open. Tracks BUG-38. Blocks CopyObject through-shim cells with a GCS destination. |
