...
| BUG-39 | P3 | dns/aws-backend | `aws-sdk-go-v2/service/route53` `ListHostedZonesByName` | Sockerless's Route 53 sim implements ListHostedZones / Get / Delete / ChangeResourceRecordSets / ListResourceRecordSets / GetChange but no `ListHostedZonesByName` route. Shim's AWS backend uses it for stateless name → HostedZoneId resolution; every shim Route 53 op other than CreateHostedZone fails 500 against sockerless. Filed [e6qu/sockerless#291](https://github.com/e6qu/sockerless/issues/291). `TestSockerless_AWSRoute53_Through_Shim_ZoneLifecycle` `t.Skip`'d pending the upstream fix. | **15.D** |
