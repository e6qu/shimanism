...
| BUG-56 | P2 | compute/azure-tf-conformance | `hashicorp/azurerm` `azurerm_linux_virtual_machine` | `azurerm_linux_virtual_machine` requires `network_interface_ids` → `azurerm_network_interface` → `azurerm_subnet`/`azurerm_virtual_network` (Microsoft.Network). A single Terraform test needs a combined azure_compute + azure_network TLS server or full ARM passthrough to sockerless for the network resources. Infrastructure (HandlerWithConfig + metadata endpoint) is in place; gap is the combined server. Mirrors DNS BUG-44 deferral. | **16.C** |
