...
**36 filed · 33 fixed · 2 open · 1 false positive.** Phase 14.B closed BUG-34 (sockerless PR #225 — SB ATOM XML admin) and BUG-36 (sockerless PR #231 — SB raw AMQP/TLS transport, lets the shim's Send/Receive lanes use `CustomEndpoint` + `TLSConfig` instead of WebSocket-dial code). Added the Azure Blob multipart, SB queue Send/Receive, and SB topic Publish/Receive lanes. BUG-35 (Container Apps pre-pull) is the only Phase-14.B residual.
