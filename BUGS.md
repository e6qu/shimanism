...
| BUG-48 | P2 | dns/coredns-backend | CoreDNS `auto` plugin | Two live conformance bugs surfaced on PR #89's first CI run. (a) CoreDNS's `auto` plugin defaults to `db\.(.*)` regex (matches `db.<zone>` filenames); the shim's K8s-peer backend writes `<zone>.db`. Fixed by passing `(.*)\.db {1}` to `directory` in the test Corefile so the auto plugin matches the shim's naming convention. (b) The auto plugin's reload doc reads: "changed zones are automatically picked up from disk only when SOA's serial changes". The backend's `PutRecordSet` / `DeleteRecordSet` preserved the existing SOA across rewrites → CoreDNS never reloaded the file → runtime updates returned NXDOMAIN. Fixed by adding `bumpSOASerial` and calling it before every `writeZoneFile` for mutations. | **15.D** |
| BUG-49 | P2 | ci/conformance-docker-steps | GitHub Actions checks workflow | Five conformance jobs (`conformance minio` / `conformance gcs` / `conformance vault` / `conformance nats` / `conformance azureblob`) each pull a container image from an external registry (`quay.io`, `docker.io`, `mcr.microsoft.com`) and immediately run it, with no retry. PR #93's first CI run failed `conformance minio` with `docker: Error response from daemon: received unexpected HTTP status: 502 Bad Gateway` (quay.io transient). Every PR is one registry hiccup away from a red CI job. Fix: add a shared `scripts/docker-pull-retry.sh` with exponential backoff and switch each `Start <X>` step to pre-pull with retry before `docker run`. | **15.C** |
