...
| BUG-56 | P2 | compute/azure-tf-conformance | `hashicorp/azurerm` `azurerm_linux_virtual_machine` | `azure_compute` frontend uses a fixed test-key bearer verifier; `azurerm` acquires tokens from a real (or sockerless) Entra ID stub whose JWKS the shim can't verify without configurable JWKS support. Mirrors BUG-44 for DNS. Unblocks `TestTerraformAzure_Compute_VMLifecycle`. | **16.C** |
| BUG-57 | P2 | compute/azure-cli-conformance | `az vm` | Same root cause as BUG-56: `azure_compute` frontend needs configurable JWKS-based bearer verification to accept tokens from sockerless's Entra stub (issued via `az login --service-principal`). Mirrors BUG-43 for DNS. Unblocks `TestAzureCLI_Compute_VMList`. | **16.C** |
