...
**38 filed · 35 fixed · 3 open · 1 false positive.** Phase 14.B PR added the Container Apps pre-pull plumbing (`scripts/run-sockerless-storage.sh`) and the Cloud Run lane, but BUG-35 (Container Apps) is **not yet closed** — CI surfaced [sockerless#244](https://github.com/e6qu/sockerless/issues/244), the Container Apps handler hardcoding `Architecture: "linux/arm64"`. Until that's fixed, the test stays default-skipped on amd64 CI. Cloud Run lane is unaffected (uses dynamic platform detection). Track A (BUG-8 + BUG-15) remains blocked on real GCP credentials. BUG-24 reverse-direction through-shim coverage gets ongoing additions per PR — three exemplar reverse cells (cache + secrets + queue, all GCP→AWS) landed in this PR.
...
| BUG-35 | P3 | functions/azure-containerapps | `armappcontainers/v3` | Container Apps lane (`TestSockerless_Azure_Functions_ContainerApps_CRUD`) needs sockerless's Container Apps handler to derive container platform dynamically. Currently `simulators/azure/containerapps_apps.go:444` hardcodes `Architecture: "linux/arm64"`, which fails on amd64 hosts (CI runners). The shim-side pre-pull plumbing landed in this PR (`scripts/run-sockerless-storage.sh`) and works once the upstream fix lands; until then the lane is default-skipped via leaving `SOCKERLESS_AZURE_CONTAINERAPPS_IMAGE` unset. Tracked upstream as [sockerless#244](https://github.com/e6qu/sockerless/issues/244). | **14.B** |
