...
- BUG-54: `DescribeNetworkInterfaces` missing from `services/compute/codegen.json`. The `hashicorp/aws` Terraform provider calls it during `aws_security_group` destroy to drain ENIs before deleting the SG. Shim returns 400 InvalidAction → destroy fails. Fix: add to codegen manifest + implement (returns empty list — stateless shim has no ENIs to drain).
- BUG-53: `DescribeVpcAttribute` missing from `services/compute/codegen.json`. The `hashicorp/aws` Terraform provider calls it after every `aws_vpc` create to read `enableDnsHostnames` + `enableDnsSupport`. Shim returns 400 InvalidAction → Terraform apply fails. Fix: add to codegen manifest, regenerate, implement in adapter (accepts, returns plausible defaults).
