...
| BUG-43 | P3 | dns/azure-cli-conformance | `az network dns` | Azure CLI's service-specific commands (`az network dns zone create`, etc.) resolve the ARM endpoint from the active `az cloud` profile rather than an `--endpoint-url` flag. shimanism's Azure DNS CLI conformance is unwritten because `az cloud register` plumbing isn't wired into the test harness. Two paths: (a) script-register a custom `shim-test` cloud per test run pointing at the harness URL, or (b) use `az rest --method ... --url ...` (the path sockerless's own CLI tests take). SDK + inmem cell already covers wire correctness. Sockerless coverage is sufficient — gap is shim-side test wiring. | **15.D** |
| BUG-45 | P3 | dns/azure-through-shim | sockerless TLS plumbing | Azure DNS through-shim sockerless test (`TestSockerless_AzureDNS_Through_Shim_ZoneLifecycle`) skipped pending the cross-cloud Apply chunk's TLS-cert plumbing. Both legs need cert trust: the shim's Azure backend (outbound to sockerless's HTTPS Azure sim) and the test harness's shim server (TLS for the SDK's Bearer-token gate). Sockerless's `public_dns.go` + `dns.go` already cover the surface; this is shim test wiring. | **15.D** |
| BUG-46 | P3 | dns/azure-tf-through-shim | shim Azure metadata + Entra ID stub | End-to-end `azurerm` Terraform Apply through the shim's Azure DNS frontend in ARM-passthrough mode needs the shim to host more than ARM proxying: the provider acquires a service-principal token from Entra ID (default `https://login.microsoftonline.com/{tenant}/...`) **before any ARM call**. Real Entra rejects test client IDs with `AADSTS700038`. To redirect token acquisition, the shim must serve a metadata endpoint (`/metadata/endpoints?api-version=...`) plus an Entra ID stub (forwarding to sockerless's Entra mock). `TestSockerless_AzureDNS_Through_Shim_Terraform_Apply` skipped pending this work. The ARM passthrough primitive itself (BUG-44) is closed and verified by `passthrough_test.go`. | **15.D** |
...
|---|---|---|---|---|
| 44 | P2 | dns/azure-arm-passthrough | this PR | Added **ARM passthrough primitive** to `internal/dns/frontends/azure_dns`: `NewWithPassthrough(d, upstream http.Handler)` forwards unmatched ARM paths to a configured upstream while DNS paths stay local. Verified by `passthrough_test.go` (resource-group / non-Microsoft.Network / providers-list paths reach upstream; DNS paths stay local; no-upstream returns honest 404 — no silent fallback). `harness.StartDNSServerAzureWithPassthrough` exposes the TLS variant + auto-generated cert PEM. End-to-end Terraform Apply through the shim is a separate problem (the provider's pre-ARM Entra ID token acquisition needs more shim infrastructure) — see BUG-46. |
