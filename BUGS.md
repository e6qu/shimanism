...
**36 filed · 32 fixed · 3 open · 1 false positive.** Phase 14.B added the Azure Blob multipart lane after sockerless PR #229 implemented block-blob staging. BUG-34 closed (sockerless PR #225 — SB ATOM XML admin); BUG-36 added (the AMQP-over-TCP follow-on tracked by sockerless#230). BUG-35 remains (Container Apps pre-pull).
...
| BUG-36 | P2 | queue+pubsub/azure-sb | `azservicebus` data client | Full SDK-driven Send/Receive (queue) and Publish/Receive (topic) lanes against sockerless need raw AMQP-over-TCP transport from the sim. Sockerless PR #229 added AMQP-over-WebSocket, but the only way for the test driver to reach a WebSocket transport is via `azservicebus.ClientOptions.NewWebSocketConn`, which forces the test to write WebSocket-dial code — leaking sockerless's transport choice into the shim's test layer. The shim's principle is: the test driver is the cloud SDK; transport beneath that is the SDK's business, not ours. Tracked upstream as [sockerless#230](https://github.com/e6qu/sockerless/issues/230). Both Azure SB queue and pubsub Send/Receive lanes remain admin-only until that closes. | **14.B** |
...
| [e6qu/sockerless#230](https://github.com/e6qu/sockerless/issues/230) — Service Bus: expose raw AMQP-over-TCP transport so callers don't need WebSocket-dial code | ◐ open. Blocks BUG-36 (Azure SB Send/Receive full SDK lanes). |
