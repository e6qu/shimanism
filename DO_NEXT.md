...
4. **14.E expansion: Service Bus Apply — implemented (this PR).** `azurerm_servicebus_namespace` + `azurerm_servicebus_queue` apply via sockerless's real Microsoft.ServiceBus ARM, then the namespace's `default_primary_connection_string` (Terraform output) drives the shim's azure_servicebus *backend* through Send/Receive against sockerless's AMQP listener (`CustomEndpoint`-pinned). Through-shim coverage for SB lives on the backend translation layer — the shim's frontend is REST/ATOM-only by design (sockerless owns the AMQP server). New `TestSockerless_E2E_AzureServiceBus_Through_Shim_ApplyTF` in `services/queue/conformance/sockerless_test.go`.
