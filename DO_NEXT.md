...
| **1.4** | ◐ | Intersection scoping + full conformance harness. (1) `services/storage/codegen.json` manifest pins the 16-op intersection. (2) `services/storage/backends/inmem/` is a real (not-fake) in-memory implementation of all 16. (3) `internal/restxml/router.go` mounts handlers with method + URI-template + required-headers + required-queries disambiguation. (4) Codegen emits `RegisterAmazonS3Routes` + payload/header decode-encode. (5) `services/storage/conformance/` exercises every op via `aws-sdk-go-v2`, `aws` CLI, and the official `hashicorp/aws` Terraform provider — all hit the shim through their standard endpoint-override paths. CI installs aws CLI + terraform so all three drivers run on every PR. PR open. |
