...
2. **15.B — 14.E residual cleanups.** First chunk (this PR): documented the terraform-aws `has_secret_string_wo` drift investigation in `docs/normalizations.md`. Shim is innocent — drift is a terraform-aws v5.100+ Read bug. Filing upstream pending user approval. Remaining 15.B work: scope SB cross-cloud (decide: build shim AMQP listener / file sockerless / formally close as out-of-intersection); resolve the two open sub-questions (N10 clamp-vs-fail, N13 tier-enum-vs-opaque).
