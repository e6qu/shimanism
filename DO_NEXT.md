...
| **4.6** | ✅ | **GCP Pub/Sub fanout backend** `services/pubsub/backends/gcp/` via `google.golang.org/api/pubsub/v1`. Multi-subscription topology — multiple subs per topic, each with its own ack-deadline; topic + subscription are first-class separate resources (Phase 3 collapsed them onto one queue; Phase 4 keeps them apart). AckIds pass through as opaque receipt handles. ChangeVisibility → ModifyAckDeadline. |
| **4.7** | ✅ | **Azure Service Bus topics backend** `services/pubsub/backends/azure/`. Same hybrid pattern as Phase 3 queue backend but with topics + subscriptions: admin.Client for Create/Delete/Get/List/Get on topics + subscriptions, azservicebus.Client.NewSender(topic) for Publish, NewReceiverForSubscription(topic, sub) for Receive, raw REST + SAS for Ack/RenewLock. Receipt handle is `<topic>|<sub>|<messageID>|<lockToken>` so the URL reconstructs without shim-side state. DeleteSubscription / HeadSubscription iterate topics to locate the owner (domain identifies subs by name only). |
| **4.8** | ✅ | **GCP Pub/Sub frontend** `internal/pubsub/frontends/gcp_pubsub/`. Same wire types as Phase 3 (reuse rule), but fanout-aware: topic ≠ subscription as separate resources, multiple subs per topic. subscription-delete no longer touches the topic. SDK conformance via `google.golang.org/api/pubsub/v1` — TestGCPSDK_PubsubFanout creates one topic + two subscriptions, publishes once, verifies each subscription receives a copy. |
| **4.9** | ✅ | **Azure Service Bus topics REST frontend** `internal/pubsub/frontends/azure_servicebus_topics/`. Routes: `PUT/DELETE/GET /{topic}` (topic admin), `PUT/DELETE/GET /{topic}/Subscriptions/{sub}` (sub admin), `POST /{topic}/messages` (publish), `POST /{topic}/Subscriptions/{sub}/messages/head` (peek-lock), `DELETE/POST /{topic}/Subscriptions/{sub}/messages/{id}/{lock}` (ack / renew). Raw-HTTP conformance test verifies fanout: one topic + two subscriptions, single publish, each subscription receives + acks its own copy. AMQP fidelity tier deferred same as Phase 3. |
| **4.10** | ✅ | Conformance matrix: `TestPubsubMatrix_{AWSFrontend,GCPFrontend,AzureFrontend}` iterates `ActiveBackends()` (inmem / nats / aws / gcp / azure with env-var gates) and drives CreateTopic → CreateSubscription × 2 → Publish → Receive on each → Ack → DeleteSubscription. The fanout assertion is built into the test (each of the two subs must independently receive the same message). AWS frontend uses both SnsURL + SqsURL (publish + receive endpoints). inmem cell green for all three frontends; the four remaining cells skip until their env gates are set. |
| **4.11** | ✅ | CLI conformance. `aws sns create-topic` + `aws sns subscribe` + `aws sns publish` against the shim's SnsURL, paired with `aws sqs receive-message` + `aws sqs delete-message` against the SqsURL. `gcloud pubsub topics+subscriptions` exercises fanout (one topic, two subs, two pulls). `az servicebus` cell ◇ skipped (ARM + AMQP, same posture as Phase 3). |
| **4.12** | ✅ | Terraform conformance. `hashicorp/google` (`google_pubsub_topic` + two `google_pubsub_subscription`) green against the GCP frontend (fanout shape verified through init→apply→destroy). `hashicorp/aws aws_sns_topic_subscription` ◇ skipped — requires `aws_sqs_queue` (backing queue) + `SetQueueAttributes` reconciliation (BUG-2 carries forward; the pubsub frontend deliberately omits the full SQS admin surface). `hashicorp/azurerm` ◇ skipped — ARM-control-plane, REST data plane only this phase. |
| **4.13** | ✅ | `cmd/shim pubsub` subcommand at `cmd/shim/pubsub.go`. Default `:9300` (SNS) + `:9301` (SQS receive surface for aws_sns frontend). Single `:9300` for gcp_pubsub / azure_servicebus_topics frontends. Selectors: -frontend (aws_sns, gcp_pubsub, azure_servicebus_topics), -backend (inmem, nats, aws, gcp, azure). Connection knobs accept flags + env vars (NATS_URL, AWS_REGION, AWS_ACCOUNT_ID, GCP_PROJECT_ID, AZURE_SERVICEBUS_CONNECTION_STRING). Version bumped to 0.5.0-phase-4. |
| **4.14** | ✅ | CI lane reuses the existing `conformance-nats` container (already starts NATS with `-js`); the test step now runs both `TestQueueMatrix` and `TestPubsubMatrix` against the same NATS instance. No new lane needed since both shimmed services share infrastructure (Phase 3 + Phase 4 NATS-backend code both depend on JetStream). |
| **4.15** | ✅ | Phase 4 closer: SDK matrix green for inmem cell of all three frontends; nats / aws / gcp / azure cells skip cleanly on env gates. CLI + TF rows green where their tooling admits endpoint override (`aws sns`+`aws sqs`, `gcloud pubsub`, `google_pubsub_*`); ◇ skipped cells documented (Azure SDK + `az servicebus` + `azurerm_servicebus_topic`, `aws_sns_topic_subscription` Terraform). `conformance-nats` lane extended (no new lane). STATUS / WHAT_WE_DID / DO_NEXT refreshed. PR pending push. |
