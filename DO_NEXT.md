...
3. **15.C + 15.D scoping doc — implemented (this PR).** `docs/phase-15-cd-scoping.md` audits both upcoming new-service sub-phases before any code lands. Per-cloud surface table, intersection candidates, spec sources, K8s peer choice, domain interface sketch, frontend/backend layout, codegen lane assignment, conformance matrix, open questions per service. Recommended order: 15.D first (smaller, fewer open questions), then 15.C.
4. **15.D — DNS service (next implementation chunk).** Per scoping doc: Route 53 + Cloud DNS + Azure DNS (public) + Azure Private DNS + CoreDNS K8s peer. Zones + standard record types. ~2-3 PRs.
5. **15.C — NoSQL key-value service.** Per scoping doc: DynamoDB + Firestore Native + Cosmos DB Table API + etcd K8s peer. Get / Put / Delete / Scan on partition key. ~3-4 PRs after 15.D lands.
