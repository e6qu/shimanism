...
| **5.1** | ◻ | Spec ingest. AWS RDS Smithy 2.0 JSON vendored at `services/rdbms/spec/aws-rds.smithy.json`. GCP Cloud SQL Admin REST reused via `google.golang.org/api/sqladmin/v1`. Azure flexible-servers reused via `armpostgresqlflexibleservers` + `armmysqlflexibleservers`. |
| **5.2** | ◻ | `internal/rdbms/domain/` neutral interface — `RDBMS` interface (10 methods + 2 Head probes) + types (`Instance`, `Snapshot`, `Connection`, `Status`, `Engine`). Async-aware: `Status` is the explicit field clients poll. |
| **5.3** | ◻ | `services/rdbms/backends/inmem/` (test fixture — fakes the async-creating→available transition with a configurable delay; `Connection` block returns a dummy `localhost:5432` host). AWS RDS frontend `internal/rdbms/frontends/aws_rds/` speaking awsQuery (form-encoded, XML responses with the RDS namespace). SDK conformance via `aws-sdk-go-v2/service/rds`. |
| **5.4** | ◻ | **CloudNativePG backend** (K8s peer) `services/rdbms/backends/cnpg/` via `k8s.io/client-go` + the cnpg-api types. Postgres engine only at this sub-phase; MySQL via MySQL Operator deferred to a follow-on if needed. CreateInstance → `Cluster` CR + master-creds `Secret`. DescribeInstance reads the cluster condition + the connection service. |
...
| **5.8** | ◻ | **GCP Cloud SQL Admin frontend** `internal/rdbms/frontends/gcp_cloudsql/`. REST/JSON; routes mirror Cloud SQL Admin REST. |
| **5.9** | ◻ | **Azure DB Admin REST frontend** `internal/rdbms/frontends/azure_dbadmin/`. ARM REST endpoint shape. |
