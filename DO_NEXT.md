...
5. **14.E expansion: SB Topics + Subscriptions Apply — implemented (this PR).** Pubsub-side analog of PR #60. `TestSockerless_E2E_AzureServiceBus_Topic_Through_Shim_ApplyTF` drives `azurerm_servicebus_namespace` + `_topic` + `_subscription` Apply via sockerless real ARM, then drives the shim's `azurepubsub` backend through Publish/Receive using the namespace's connection-string output.
6. **14.E expansion (next concrete chunk):** Storage Queues (`azurerm_storage_queue`) — extends PR #58's storage pattern; Storage Queues data plane is HTTP, so the shim's azure_blob/queue frontend would actually be on the path again (closer to the storage/KV pattern). Tentatively recommended after this PR.
