...
**Phase 18 — Container Registry is in 18.D closeout.** OCI Distribution `/v2/` data plane (shared hand-written router) + ECR/AR/ACR control planes + connected backends. See [docs/phase-18-scoping.md](docs/phase-18-scoping.md). 18.A, 18.B, and 18.C are complete: GCP AR, AWS ECR, and Azure ACR frontends all have SDK/CLI/Terraform control-plane coverage plus go-containerregistry OCI data-plane coverage. 18.D PR1 (#139) merged the real CNCF `distribution` backend; PR2 (#140) merged the real AWS ECR backend and BUG-61 fix. Current branch keeps the remaining work in one PR per user direction: GCP AR + Azure ACR connected backends, registry sockerless lanes, and closeout docs.
...
- [x] **PR2 — AWS ECR connected backend.** `services/registry/backends/aws_ecr` uses a real `*ecr.Client` for repository lifecycle, tag reads, image list, image delete, and `GetAuthorizationToken`; OCI blob/manifest/tag traffic delegates to the real ECR `/v2/` endpoint with the Basic credential ECR returns. No repository/image sidecar state. Also fixed BUG-61 by returning ECR `BatchDeleteImage.failures[]` instead of dropping per-image delete failures. Merged PR #140.
- [ ] **Single closeout PR (current):** GCP Artifact Registry + Azure ACR connected backends; registry sockerless lanes; `services/registry/INTERSECTION.md`; `services/registry/APPLY_INTERSECTION.md`; PLAN/STATUS/WHAT_WE_DID updates. Registry sockerless `/v2/` gaps remain loud BUG skips: AWS ECR has no `/v2/` (BUG-64), GCP AR chunk `PATCH` returns 405 (BUG-65), and Azure ACR upload start returns 404 (BUG-66). Ask user before filing the specific upstream issues.
