...
1. **15.A — Normalisations contract doc — first cut implemented (this PR).** New `docs/normalizations.md` enumerating the eight rules already implemented in code (N1–N8: secret value-less create, secret version identity, tags-vs-labels, description encoding, queue ↔ topic+subscription, region naming, storage version identity, storage metadata-vs-tags). Cross-referenced from PHILOSOPHY.md / AGENTS.md / docs/architecture.md. Open items (soft-delete grace, RDBMS engine version naming + connection string, cache cluster mode, functions runtime mapping, API Gateway stages-vs-configs-vs-products) tracked at the bottom of `normalizations.md` for follow-on 15.A PRs.
