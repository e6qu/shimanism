...
| **10.2** | ✅ | Create-then-Read drift audit per service. Apply test scaffolding in tree for all 8 services. Active drift assertions: storage / secrets / apigateway / **queue (AWS, after BUG-2 closed)** / **pubsub (AWS, after SetTopicAttributes wired)**. Documented skips with BUG pointers: rdbms, cache, functions (each blocked on its own provider-side reconcile + missing metadata). |
