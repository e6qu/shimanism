...
2. **15.B — 14.E residual cleanups.** PR #75 documented the `_wo` drift (user opted not to file upstream). This PR resolves the **N10 clamp-vs-fail sub-question** in favour of *fail*: the GCP queue backend returns `domain.InvalidArgument` on `VisibilityTimeoutSeconds > 600` instead of silently clamping. Topic rollback on CreateQueue failure. Aligns with shimanism's "never lie" rule. Remaining 15.B: N13 tier-enum-vs-opaque decision, SB cross-cloud scoping (build shim AMQP / file sockerless / formally close as out-of-intersection).
