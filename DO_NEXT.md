...
| **1.13** | ◐ | CI conformance matrix lights up minio (MinIO container), gcs (fake-gcs-server), azureblob (Azurite). Each lane runs `TestConformance_AllBackends` with the env var its docker container exposes; the factory's other-backend skips keep each lane focused. Remaining: AWS lane (needs real AWS creds; deferred to Track A), Terraform-against-real-backend lane (deferred), K8s peer end-to-end (manifests ship; cluster CI deferred). Piled on PR #6. |
| **1.14** | ◻ | **GCS frontend.** Phase 1 now requires every frontend × every backend (PLAN.md principle 11). Per locked-in decision #11 (reuse-over-reinvention) the first move is to **reuse `google.golang.org/api/storage/v1` raw wire types** (auto-generated from the GCS Discovery doc — same source the official SDK uses) rather than re-emitting equivalents. Tasks: vendor or import the raw types; emit only the routing + dispatch + auth layer ourselves from the Discovery doc; `internal/storage/frontends/gcs/` adapter wrapping `domain.Storage`; GCS-specific probe handling (the equivalent of S3's bucket-config probes for what `gcloud` / `hashicorp/google` query); auth via `golang.org/x/oauth2` token verification; conformance via `cloud.google.com/go/storage` SDK + `gcloud` CLI + `hashicorp/google` Terraform provider against all four backends. |
| **1.15** | ◻ | **Azure Blob frontend.** Per locked-in decision #11, first move is to **evaluate `oapi-codegen` server-stub generation** against the Azure OpenAPI v3 spec from `Azure/azure-rest-api-specs/specification/storage`; reuse the SDK's internal `generated/` types where they round-trip cleanly; emit the routing + SharedKey/SAS verifier ourselves only if oapi-codegen output can't match the shim's handler shape after reasonable adapter glue. Conformance via `azure-sdk-for-go/sdk/storage/azblob` SDK + `az` CLI + `hashicorp/azurerm` Terraform provider against all four backends. |
