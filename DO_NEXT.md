...
| **10.2** | ✅ | Create-then-Read drift audit per service. Apply test scaffolding in tree for all 8 services. Active drift assertions: storage / secrets / apigateway / queue (AWS) / pubsub (AWS) / functions (AWS) / **rdbms (GCP, after BUG-16 closed)**. Documented skips with BUG pointers: cache (BUG-2-class for AWS, BUG-16-family for GCP), AWS rdbms (Modify reconcile + subnet/parameter-group metadata), Azure cells across services (Azure-AsyncOperation URLs). |
