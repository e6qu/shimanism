...
| **1.14** | ◻ | **GCS frontend.** Phase 1 now requires every frontend × every backend (PLAN.md principle 11). Tasks: ingest GCP Discovery doc / protobuf for `storage.v1`, extend codegen pipeline to emit GCS-shaped server stubs (JSON wire, OAuth2 bearer auth), `internal/storage/frontends/gcs/` adapter wrapping `domain.Storage`, GCS-specific probe handling (the equivalent of S3's bucket-config probes for what `gcloud` / `hashicorp/google` query), conformance via `cloud.google.com/go/storage` SDK + `gcloud` CLI + `hashicorp/google` Terraform provider against all four backends. |
| **1.15** | ◻ | **Azure Blob frontend.** Extend codegen for Azure OpenAPI v3, `internal/storage/frontends/azure_blob/` adapter, SharedKey/SAS auth, conformance via Azure SDK + `az` CLI + `hashicorp/azurerm` Terraform provider against all four backends. |
