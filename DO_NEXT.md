...
4. **14.E expansion: Service Bus Apply — implemented (this PR).** sockerless#276 closed via #277 (networkRuleSets + disaster recovery list + migration config 404 + queue/topic/subscription ARM delete-response shape). `TestSockerless_E2E_AzureServiceBus_Through_Shim_ApplyTF` drives `azurerm_servicebus_namespace` + `azurerm_servicebus_queue` Apply via sockerless real ARM, reads the namespace's `default_primary_connection_string` Terraform output, feeds it into the shim's azure_servicebus backend, and runs Send/Receive against sockerless's AMQP listener. Through-shim coverage lives on the backend translation layer (shim's SB frontend is REST/ATOM-only by design).
