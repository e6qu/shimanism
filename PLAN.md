...
| 12.A.11+ | ◻ | Adapter migrations for the remaining 4 Azure frontends whose gen files exist (queue/azure_servicebus, pubsub/azure_servicebus_topics, cache/azure_redis, functions/azure_containerapps, apigateway/azure_apim). Each requires per-frontend wiring through `gen.ServerInterface`; the existing hand-written frontends work and conformance stays green, so this is dispatch-consistency work rather than fidelity. |
| 12.A.x | ⏸ | Azure PostgreSQL ARM (rdbms) — duplicate-typename `HighAvailabilityMode` (standalone enum vs. inline `HighAvailability.properties.mode` enum) blocks codegen. Needs either an emitter-level honoring of x-ms-enum.name or a spec pre-process pass that rewrites inline enums to `$ref`s. |
| 12.A.x | ⏸ | Azure Blob Storage — spec uses `x-ms-paths` (Azure extension that disambiguates same-URL ops by query parameter) instead of `paths`. oapi-codegen doesn't understand it; generated server has zero operations. Needs a pre-process pass that flattens `x-ms-paths` into `paths` with per-operation disambiguation. |
