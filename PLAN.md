...
| 11.8 | ◐ | **Pubsub (SNS).** **11.8a** ✅ `awsQuery` emitter path landed (`internal/awsquery/` + `template_awsquery.tmpl`); 7 unit tests. **11.8b** ✅ SNS manifest extended to 11 ops (SetTopicAttributes + ListTagsForResource). **11.8c** ✅ Emitter decodes `map<string,string>` (`Field.entry.N.{key,value}`) + `list<string>` (`Field.member.N`) collection patterns. **11.8d** ◻ SNS adapter migration. `MessageAttributes` (map of struct) still needs nested decoding — handled via direct r.Form parsing in adapter OR emitter extension for `map<string,struct>`. |
...
| 11.11 | ◐ | **rdbms (RDS).** Gen file in tree (~94 KB). Adapter migration pending — collection decoding for `list<string>` (SecurityGroups, AvailabilityZones, etc.) covered by 11.8c. |
| 11.12 | ◐ | **Cache (ElastiCache).** Same — gen file in tree; adapter pending. |
