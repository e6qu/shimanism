...
| 12.9 | ◐ | Closer: this PR's substantive landing is the spec-driven *toolchain* for both clouds — **8/8 Azure specs codegen end-to-end** + **8/8 GCP route inventories with Match() helper** + `make codegen` pipeline runs Smithy → Azure → GCP. Adapter migrations that wire generated types into each frontend's request decoding/response encoding are mechanical follow-ons (the existing hand-written frontends work and conformance stays green; `azure_keyvault` ships fully migrated as the reference implementation). The remaining adapter migrations + cross-cloud Apply cell expansion + spec-drift CI lane go in a Phase 13-shaped follow-on PR. |
