...
| 12.A.15 | ✅ | `flattenXMSPaths` preprocessor moves `x-ms-paths` entries into `paths` with the same key (OpenAPI treats path keys as opaque strings; distinct query strings → distinct entries). Partial Blob unblock — Blob has additional spec quirks (refs into `#/components/schemas/AccessTier` the v2→v3 converter expects as parameter refs); spec stays unvendored pending follow-on preprocessing. The flattener works on simpler `x-ms-paths`-using specs. |
| 12.A.x | ⏸ | Azure Blob Storage — `x-ms-paths` flattened OK by 12.A.15, but the spec has additional ref-shape quirks (refs into `#/components/schemas/AccessTier` expected as parameter refs). Needs another per-spec preprocessor pass that reshapes those refs. The hand-written azure_blob frontend works today; this is dispatch-consistency follow-on. |
...
| 12.B.1 | ✅ | Vendor + generate Discovery routing for all 8 GCP services (storage / secrets / queue / pubsub / rdbms / cache / functions / apigateway). 8 `services/<svc>/gen/gcp/*.gen.go` files; 593 routes total. `make codegen` picks up `gcp-codegen.json` manifests automatically. |
