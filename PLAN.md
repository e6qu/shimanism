...
| 12.A.x | ⏸ | Azure Blob Storage — spec uses `x-ms-paths` (Azure extension that disambiguates same-URL ops by query parameter) instead of `paths`. oapi-codegen + std-net-http ServeMux dispatch only on (method, path-pattern), no query awareness. Needs custom dispatcher + spec preprocessing (flatten x-ms-paths into synthetic path keys + a request-rewriting middleware) or a different routing layer. The hand-written azure_blob frontend works today; this is dispatch-consistency follow-on. |
...
| 12.9 | ◻ | Closer: 8/8 Azure + GCP frontends spec-driven at the adapter layer; cross-cloud Apply matrix has one honest cell per service; per-service `MIGRATION.md` updated with the runnable recipe; spec-drift CI lane (Renovate coverage) candidate. Adapter migrations are tractable mechanical work — the toolchain has been validated end-to-end on every spec that doesn't hit a known per-spec preprocessing wrinkle (PostgreSQL duplicate-typename, Blob x-ms-paths). |
