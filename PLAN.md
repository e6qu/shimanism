...
6. **Kubernetes is a first-class backend.** Every service has a K8s peer on equal footing with the three clouds. When a suitable third-party OSS peer exists (MinIO for storage, Vault for secrets, NATS / CloudNativePG / Knative / etc.), use it. When none fits, the in-tree [`peers/shimanism/`](peers/shimanism/) package provides the common-denominator K8s peer — one binary, one Store interface, namespace-scoped so it serves multiple shim services from a single deployment.
...
| 13 | **In-tree K8s peer when OSS doesn't fit** | The K8s-peer slot for every shimmed service prefers a third-party OSS project (MinIO for storage, Vault for secrets, NATS for queues, CloudNativePG for RDBMS, Knative for functions, Envoy Gateway for API GW). When no OSS project fits cleanly, the in-tree [`peers/shimanism/`](peers/shimanism/) package fills the slot. It is a **single, common-denominator** service — versioned, named binary objects with structured metadata, multi-namespace addressing — not a fleet of per-service shim peers. One binary covers every gap. Released as a separate Go module so operators can deploy / upgrade it independently of the shim binary. |
