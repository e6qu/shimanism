...
| 11.3 | ◐ | AWS Secrets Manager service migration. **11.3a** ✅ generalised `make codegen` across all manifests; shipped generated `services/secrets/gen/aws_secretsmanager.gen.go` (11 ops) + `services/queue/gen/aws_sqs.gen.go` (9 ops); `cmd/codegen` treats `unsupported wire protocol` as a skip not a fail. **11.3b** ✅ `awsjson.EpochTime` runtime type + emitter awareness of epoch-seconds timestamp serialisation (awsJson1_x / aws-query). **11.3c** ◻ wire the generated router into `harness.StartSecretsServerAWS`; write the per-op adapter implementing `gen.SecretsManagerBackend`; delete the 691-line hand-written `internal/secrets/frontends/aws_secretsmanager/handlers.go`. |
