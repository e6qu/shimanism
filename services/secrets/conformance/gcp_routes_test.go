...
// TestGCPRoutes_Secrets_EveryRouteRoundTrips synthesizes a sample path
...
func TestGCPRoutes_Secrets_EveryRouteRoundTrips(t *testing.T) {
	for _, r := range gcpgen.Routes {
		path := gcpgen.BasePath + "/" + r.URIPattern
		path = expandRouteTemplateSecrets(path)
...
// expandRouteTemplateSecrets substitutes URI-template variables with
// sample values; both {var} and {+var} become "x".
func expandRouteTemplateSecrets(t string) string {
	out := []byte{}
	for i := 0; i < len(t); {
		if t[i] == '{' {
			end := i + 1
