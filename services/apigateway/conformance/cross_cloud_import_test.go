...
func TestCrossCloudImport_Roundtrip_APIGwAWStoGCP(t *testing.T) {
	...
	runTf := func(args ...string) ([]byte, []byte, error) {
		...
		cmd.Env = append(os.Environ(),
			...
			"TF_PLUGIN_CACHE_DIR="+terraformPluginCacheDirAPIGWCC(),
		)
...
func terraformPluginCacheDirAPIGWCC() string {
	d := filepath.Join(os.TempDir(), "shim-apigateway-cc-tf-plugin-cache")
