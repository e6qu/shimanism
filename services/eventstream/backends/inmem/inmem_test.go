...
func TestTopicLifecycleAndList(t *testing.T) {
	...
	topic, err := b.CreateTopic(ctx, "events.alpha", domain.CreateTopicOptions{
		PartitionCount: 2,
		Retention:      time.Hour,
	...
	if topic.Name != "events.alpha" || topic.PartitionCount != 2 || topic.Retention != time.Hour || !topic.CreatedAt.Equal(createdAt) {
		t.Fatalf("unexpected topic: %#v", topic)
	}

	if _, err := b.CreateTopic(ctx, "events.alpha", domain.CreateTopicOptions{PartitionCount: 1}); !domain.IsAlreadyExists(err) {
		t.Fatalf("CreateTopic duplicate error = %v, want already exists", err)
	}
	if _, err := b.CreateTopic(ctx, "events.beta", domain.CreateTopicOptions{PartitionCount: 1}); err != nil {
		t.Fatalf("CreateTopic events.beta: %v", err)
	}
	if _, err := b.CreateTopic(ctx, "metrics", domain.CreateTopicOptions{PartitionCount: 1}); err != nil {
		t.Fatalf("CreateTopic metrics: %v", err)
	}

	describe, err := b.DescribeTopic(ctx, "events.alpha")
	if err != nil {
		t.Fatalf("DescribeTopic: %v", err)
	}
	describe.Tags["owner"] = "mutated"
	describeAgain, err := b.DescribeTopic(ctx, "events.alpha")
	if err != nil {
	...
	page1, err := b.ListTopics(ctx, domain.ListTopicsOptions{Prefix: "events.", MaxResults: 1})
	if err != nil {
	...
	page2, err := b.ListTopics(ctx, domain.ListTopicsOptions{Prefix: "events.", NextToken: page1.NextToken})
	if err != nil {
	...
	if err := b.DeleteTopic(ctx, "events.alpha"); err != nil {
		t.Fatalf("DeleteTopic: %v", err)
	}
	if _, err := b.DescribeTopic(ctx, "events.alpha"); !domain.IsNotFound(err) {
...
func TestProduceFetchAndOffsets(t *testing.T) {
	...
	if _, err := b.CreateTopic(ctx, "events", domain.CreateTopicOptions{PartitionCount: 2}); err != nil {
		t.Fatalf("CreateTopic: %v", err)
	}
	...
	meta, err := b.Produce(ctx, "events", 1, input)
	if err != nil {
	...
	records, err := b.Fetch(ctx, "events", 1, 1, 2)
	if err != nil {
	...
	all, err := b.Fetch(ctx, "events", 1, 0, 0)
	if err != nil {
	...
	all[0].Value[0] = 'x'
	allAgain, err := b.Fetch(ctx, "events", 1, 0, 0)
	if err != nil {
	...
	bounds, err := b.ListOffsets(ctx, "events", 1)
	if err != nil {
...
func TestConsumerOffsets(t *testing.T) {
	ctx := context.Background()
	b := New()
	if _, err := b.CreateTopic(ctx, "events", domain.CreateTopicOptions{PartitionCount: 1}); err != nil {
		t.Fatalf("CreateTopic: %v", err)
	}
	if _, err := b.Produce(ctx, "events", 0, []domain.ProducerRecord{{Value: []byte("one")}}); err != nil {
		t.Fatalf("Produce: %v", err)
	}

	if _, err := b.FetchCommittedOffset(ctx, "workers", "events", 0); !domain.IsInvalidInput(err) {
		t.Fatalf("FetchCommittedOffset missing group error = %v, want invalid input", err)
	}
	if err := b.CommitOffset(ctx, "workers", "events", 0, 2); !domain.IsInvalidInput(err) {
		t.Fatalf("CommitOffset beyond latest error = %v, want invalid input", err)
	}
	if err := b.CommitOffset(ctx, "workers", "events", 0, 1); err != nil {
		t.Fatalf("CommitOffset: %v", err)
	}
	offset, err := b.FetchCommittedOffset(ctx, "workers", "events", 0)
	if err != nil {
	...
	if err := b.DeleteTopic(ctx, "events"); err != nil {
		t.Fatalf("DeleteTopic: %v", err)
	}
	if _, err := b.FetchCommittedOffset(ctx, "workers", "events", 0); !domain.IsNotFound(err) {
...
func TestRetentionPrunesOldRecords(t *testing.T) {
	...
	b.now = func() time.Time { return now }
	if _, err := b.CreateTopic(ctx, "events", domain.CreateTopicOptions{PartitionCount: 1, Retention: 5 * time.Second}); err != nil {
		t.Fatalf("CreateTopic: %v", err)
	}
	meta, err := b.Produce(ctx, "events", 0, []domain.ProducerRecord{
	...
	if _, err := b.Fetch(ctx, "events", 0, 0, 1); !domain.IsInvalidInput(err) {
		t.Fatalf("Fetch before earliest error = %v, want invalid input", err)
	}
	records, err := b.Fetch(ctx, "events", 0, 1, 0)
	if err != nil {
	...
	bounds, err := b.ListOffsets(ctx, "events", 0)
	if err != nil {
		t.Fatalf("ListOffsets: %v", err)
	}
	...
	meta, err = b.Produce(ctx, "events", 0, []domain.ProducerRecord{{Value: []byte("newer")}})
	if err != nil {
...
func TestValidation(t *testing.T) {
	ctx := context.Background()
	b := New()
	if _, err := b.CreateTopic(ctx, "", domain.CreateTopicOptions{PartitionCount: 1}); !domain.IsInvalidInput(err) {
		t.Fatalf("CreateTopic empty name error = %v, want invalid input", err)
	}
	if _, err := b.CreateTopic(ctx, "bad topic", domain.CreateTopicOptions{PartitionCount: 1}); !domain.IsInvalidInput(err) {
		t.Fatalf("CreateTopic whitespace name error = %v, want invalid input", err)
	}
	if _, err := b.CreateTopic(ctx, "events", domain.CreateTopicOptions{PartitionCount: 0}); !domain.IsInvalidInput(err) {
		t.Fatalf("CreateTopic zero partitions error = %v, want invalid input", err)
	}
	if _, err := b.CreateTopic(ctx, "events", domain.CreateTopicOptions{PartitionCount: 1}); err != nil {
		t.Fatalf("CreateTopic: %v", err)
	}
	if _, err := b.Fetch(ctx, "events", 1, 0, 1); !domain.IsInvalidInput(err) {
		t.Fatalf("Fetch bad partition error = %v, want invalid input", err)
	}
	if _, err := b.Fetch(ctx, "events", 0, -1, 1); !domain.IsInvalidInput(err) {
	...
	if _, err := b.ListTopics(ctx, domain.ListTopicsOptions{NextToken: "x"}); !domain.IsInvalidInput(err) {
		t.Fatalf("ListTopics bad token error = %v, want invalid input", err)
	}
	if _, err := b.ListTopics(ctx, domain.ListTopicsOptions{NextToken: "99"}); !domain.IsInvalidInput(err) {
