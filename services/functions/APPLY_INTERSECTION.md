...
| `role` / IAM role (AWS) | ⚠ | Closed in Phase 10.3 (BUG-13). `domain.Function` gained `Role`. AWS frontend round-trips it. **Non-AWS backends reject non-empty `Role` with `InvalidArgument`** — the destination's identity model (Cloud Run service-account, Container Apps managed identity, Knative ServiceAccount) doesn't translate from an AWS IAM role ARN. Cross-cloud migration tools rebind identity at the destination (a follow-on phase); the shim refuses to silent-drop. |
| `publish` (AWS) | ⚠ | Closed in Phase 10.3 (BUG-13). `domain.Function` gained `Publish`. AWS frontend emits `Version = "$LATEST"` (Publish=false) or `"1"` (Publish=true). **Non-AWS backends reject `Publish=true` with `InvalidArgument`** (Cloud Run / Container Apps / Knative revisions are atomic — no separate "published-version" concept). |
