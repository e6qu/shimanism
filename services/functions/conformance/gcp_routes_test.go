...
// TestGCPRoutes_Functions_EveryRouteRoundTrips synthesizes a sample path
...
func TestGCPRoutes_Functions_EveryRouteRoundTrips(t *testing.T) {
	for _, r := range gcpgen.Routes {
		path := gcpgen.BasePath + "/" + r.URIPattern
		path = expandRouteTemplateFunctions(path)
...
// expandRouteTemplateFunctions substitutes URI-template variables with
// sample values; both {var} and {+var} become "x".
func expandRouteTemplateFunctions(t string) string {
	out := []byte{}
	for i := 0; i < len(t); {
		if t[i] == '{' {
			end := i + 1
