...
PR #64 extended the pattern to secrets: `TestSockerless_E2E_AzureKV_Through_Shim_ApplyTF_BackendAWS` runs the same azurerm KV Apply as PR #59, but the shim's azure_keyvault backend is AWS Secrets Manager talking to sockerless's AWS sim. This PR mirrors that on the GCP corner: `TestSockerless_E2E_AzureKV_Through_Shim_ApplyTF_BackendGCP` runs the same Apply against sockerless's GCP sim via the shim's GCP Secret Manager backend. Together with PR #59 (inmem) and PR #64 (AWS), this PR closes the Azure-source KV row of the cross-cloud Apply matrix. End-to-end this realises the cross-cloud Apply promise for secrets across both destination clouds: write Azure-shape Terraform, get an AWS- or GCP-shape secret on the destination cloud.
