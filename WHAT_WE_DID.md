...
This PR extends the pattern to secrets: `TestSockerless_E2E_AzureKV_Through_Shim_ApplyTF_BackendAWS` runs the same azurerm KV Apply as PR #59, but the shim's azure_keyvault backend is AWS Secrets Manager talking to sockerless's AWS sim. First cross-cloud Apply for secrets; verifies the secret value the Azure-shaped Terraform put through the shim lands in the AWS-shaped store. End-to-end this is the cross-cloud Apply promise extended from storage to secrets: write Azure-shape Terraform, get an AWS-shape resource on the destination cloud — and now for KV secrets too.
