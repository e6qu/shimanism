...
**Follow-on cells: cross-cloud backends for the existing Apply cells (this PR, after PR #61).** The four 14.E Apply cells so far (#58/#59/#60/#61) all land Azure terraform → shim → *inmem* backend (or sockerless AMQP for SB). That demonstrates honest azurerm-↔-shim compatibility but doesn't yet exercise the cross-cloud translation the shim is for. `TestSockerless_E2E_AzureBlob_Through_Shim_ApplyTF_BackendAWS` runs the same azurerm storage account + container Apply as PR #58, but the shim's azure_blob backend is the AWS S3 backend talking to sockerless's AWS sim — verifies the "container" the Azure-shaped Terraform created lands as an S3 bucket in the AWS-shaped store. End-to-end this is the cross-cloud Apply promise: write Azure-shape Terraform, get an AWS-shape resource on the destination cloud.
