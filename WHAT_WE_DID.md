...
This PR opens the second source row: AWS-source storage Apply. `TestSockerless_E2E_AWSS3_Through_Shim_ApplyTF_BackendGCS` drives `hashicorp/aws` Terraform Apply through the shim's aws_s3 frontend (SigV4-verified), then through the shim's GCS backend to sockerless's GCP sim. The bucket the `aws_s3_bucket` resource declared lands as a GCS bucket in sockerless. AWS source is mechanically simpler than Azure source: S3 is one-tier (no separate ARM step), the SigV4 path is signed HTTP (no TLS-cert plumbing for the shim's listener), and the shim's URL can be a random httptest port (no fixed-port gymnastics, since the URL is read after-the-fact for the provider's `endpoints { s3 = ... }` block).
