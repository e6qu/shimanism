...
**Follow-on cells: Service Bus Apply (this PR, after PR #59).** Adding `azurerm_servicebus_namespace` + `azurerm_servicebus_queue` Apply was the third 14.E expansion. The shape is different from storage/KV: shimanism's `internal/queue/frontends/azure_servicebus` is REST/ATOM-only by design — AMQP listening lives in sockerless, not the shim. So the shim's *frontend* isn't on the data-plane path here; the through-shim coverage is on the *backend* translation layer. The shim's `azurequeue` backend parses the SAS connection string azurerm emits and dispatches via `azservicebus.Client` with `CustomEndpoint` pinned at sockerless's AMQP port. Test reads the namespace's `default_primary_connection_string` as a Terraform output and feeds it straight into the backend's config — no hand-built connection strings, no hardcoded SAS keys. Validates that azurerm-emitted SB connection strings parse cleanly in the shim's backend and end-to-end Send/Receive works against the AMQP server sockerless brings up. The clarifying realisation: not every shimmed service has both frontend-translation AND backend-translation through-shim coverage; for SB the meaningful through-shim test exercises the backend layer.
