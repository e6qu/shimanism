...
CI on the closeout branch showed that sockerless#450 / BUG-64 is fixed on current sockerless main: AWS ECR `/v2/` now responds. The branch replaced the old AWS ECR gap probe with a full through-shim image push/pull attempt using the real AWS backend and OCI client, and moved BUG-64 out of the open bug set. That immediately surfaced BUG-67: sockerless AWS ECR returns 400 on the normal OCI manifest `HEAD` probe for a missing tag, filed upstream as [sockerless#465](https://github.com/e6qu/sockerless/issues/465).

After sockerless PR #475 landed, the registry lane was rebuilt from current sockerless main (`3d457dd`) and rerun. GCP Artifact Registry and Azure ACR now complete full through-shim push/pull, so the local BUG-65 and BUG-66 skip gates were removed instead of carried as stale fallbacks. AWS ECR remains skipped only on BUG-67. The validation sweep also exposed two local follow-ups before any fixes: BUG-68 for expired cached sockerless TLS cert reuse, and BUG-69 for the KMS Terraform lane hiding stdout when `detailed-exitcode` returns 2.
...
ACR's main asymmetry is still N30: Azure repositories are implicit. The ARM `registries/{name}` resource is the registry host, not a repository. The current frontend returns a minimal ARM registry-host envelope for clients that need to discover `loginServer`, but repository lifecycle and image inventory come from the ACR/OCI data plane, not invented host-side state. The official `armcontainerregistry` SDK forced one useful ARM-fidelity correction: synchronous `DELETE registries/{name}` must return `204 No Content`, not a headerless `202`, because Azure pollers require a polling URL for async deletes. `az acr` and `azurerm_container_registry` lanes are present behind the same sockerless Entra/passthrough setup used by prior Azure ARM services. PR #138 merged, closing 18.C.

18.D now starts with connected backends. The pragmatic first PR is the CNCF `distribution` peer: it is the reference OCI registry, needs no cloud credentials, and tests the shim as a real verifying proxy instead of only against inmem. The hard rule for this PR: no sidecar catalog, no shim-owned repository map, no fake empty repositories. Repository and manifest answers must come from `distribution`'s own `/v2/`/catalog/tag/manifest APIs, or fail loudly when the peer cannot answer.

The first 18.D backend now follows that rule. `services/registry/backends/distribution` is a real HTTP client for Distribution: catalog and tag listing come from `/v2/_catalog` and `tags/list`, blobs/manifests stream through the upstream registry, and chunked uploads carry Distribution's own upload `Location` as the backend session handle instead of storing an upload map in the shim. Empty repository creation returns `ErrNotSupported`; Distribution cannot create one, and the shim does not invent a catalog entry. The live conformance gate is `TestDistributionBackend_GCPAR_ImagePushPull`, which drives go-containerregistry through the GCP Artifact Registry-shaped frontend into a real registry when `SHIMANISM_DISTRIBUTION_URL` is set.

This work also surfaced BUG-62: connected registries may not expose creation/upload timestamps, and the frontends were formatting zero times as year 0001. The fix is to omit optional source-cloud time fields when the backend has no real timestamp, and to map registry `ErrNotSupported` to an ECR client error rather than a generic server failure.

The current 18.D cloud-backend chunk starts with AWS ECR because ECR has the cleanest control/data split for the existing domain: repository lifecycle, repository tags, image inventory, and image delete come from the official `aws-sdk-go-v2/service/ecr` client; OCI blob/manifest/tag operations use ECR's own Docker Registry endpoint after calling `GetAuthorizationToken` and decoding the returned Basic credential. The Distribution backend grew a request-auth hook so cloud data planes can compose with the real `/v2/` client without adding shim-owned state. This chunk also closes BUG-61: ECR `BatchDeleteImage` now reports per-image `failures[]` instead of silently dropping backend delete errors.
...
The phase's value was less in the happy path than in what the conformance contract *forced into the open*. Four lessons worth carrying forward:

**Honesty beats convenience (keyRing, BUG-58).** The GCP frontend had been returning synthetic 200s for `keyRings.create/get` — any ring "existed", even one never created. Driving it with `gcloud`/Terraform (19.D PR1) and the user's "no fake behaviour" push exposed it. The fix promoted keyRing to a *non-optional* `domain.KMS` capability: inmem (the test backend-of-record) and native GCP track rings for real, while AWS and Azure — which have no honest data-plane home for a keyRing — return `ErrNotSupported`. Determining the "app intersection" was the crux: the key-container concept is GCP-data-plane-only (Azure's vault is control-plane, AWS/K8s have none), so it's out of the cross-cloud intersection and handled frontend-local. A new test asserts unknown ring → real 404, re-create → 409.

**A test that never runs is worse than no test (BUG-59).** The AWS KMS sockerless lane shipped in 19.C had *never executed in CI* — `services/kms/conformance` was simply missing from the sockerless runner's package list. Wiring it in surfaced two latent shim bugs the dead lane had hidden: the AWS backend client didn't trust sockerless's self-signed cert, and — more substantively (BUG-60) — the AWS backend's `DescribeKey` returned no tags. Real AWS KMS `DescribeKey` doesn't carry tags (they need a separate `ListResourceTags` call), so a tagged `aws_kms_key` made the provider poll tag propagation until timeout ("Still creating…"). The inmem TF test had used an untagged key, so the gap sat invisible until the real backend ran.

**`gcloud kms` and the data-integrity contract.** The GCP CLI/TF lane (19.D PR1) revealed the frontend was omitting Cloud KMS's CRC32C integrity fields; `gcloud` computes and *verifies* them, rejecting an otherwise-valid response as "corrupted in-transit". The fix computes real Castagnoli CRC32C, verifies request-side checksums (including the empty-AAD checksum `gcloud` always sends), and decodes request bytes tolerant of URL-safe base64 (proto3 JSON). It also needed `cryptoKeyVersions` list/get/`:destroy` for `google_kms_crypto_key`.

**Sockerless gaps: report, don't work around.** Four sockerless fidelity gaps surfaced and were filed upstream with exact line numbers and suggested fixes, all closed the same day: KV secret-version listing ordered by random UUID not creation time (#407→PR #412), KMS tagging broken (#413→PR #415), no GCP Cloud KMS simulator at all (#419→PR #422), and Azure KV 405-ing version-less key crypto (#423→PR #425). Each fix was validated locally against the rebuilt sim before un-gating the lane — not push-and-hope.
...
- **Backend (`services/nosql/backends/azure/azure.go`).** Passthrough via `github.com/Azure/azure-sdk-for-go/sdk/data/aztables`. Per N18, schema lives in a reserved Cosmos table named `shimtables` — each `domain.Table` becomes one entity in that table keyed by `PartitionKey=metadata, RowKey=<tableName>` carrying `partitionKey` / `sortKey` / `description` / `tags` (the last as a JSON-encoded string since Cosmos has no nested map type). The metadata write lives in Cosmos itself, not the shim — no sidecar storage.
- **Cosmos requires string keys; the shim preserves type fidelity.** Cosmos's `PartitionKey` and `RowKey` columns are string-only. To carry `domain.Value`s of every type through Cosmos's string-only keys (so cross-cloud Apply with a DynamoDB number-keyed table round-trips), the backend encodes values with typed prefixes — `s:abc` / `n:42` / `b:1` / `x:<base64>` / `_:null` — matching the AWS page-token codec. A tableless write to a sort-key-less table uses `_:none` as the sentinel RowKey because Cosmos refuses empty RowKey strings.
- **Frontend (`internal/nosql/frontends/azure_cosmos_tables/server.go`).** Dispatches the OData v3 Tables-on-Storage protocol: `POST /Tables` (CreateTable), `GET /Tables` (ListTables), `DELETE /Tables('name')` (DeleteTable), and per-entity `POST/PUT/MERGE/GET/DELETE /<table>(PartitionKey='x',RowKey='y')`. List + Query both route to `domain.Scan` when there's no `$filter`, or to `domain.Query` when `$filter` matches the `PartitionKey eq '<x>'` shape (with optional `RowKey ge/lt` for prefix). SDK Tables clients don't know about `domain.Table`'s schema — `CreateTable` from the wire defaults to `PartitionKeyName="PartitionKey"` + `SortKeyName="RowKey"` (the wire-native names), and the frontend rejects writes to `shimtables` to prevent users from corrupting shim state.
- **`internal/azuresharedkey/` extended for the Tables variant.** Cosmos and Storage Tables sign requests with the `SharedKeyLite` scheme (not the full `SharedKey` used by Blob / Queue / File), and the canonical string is different: `x-ms-date + "\n" + canonicalizedResource`, where `canonicalizedResource = /account u.EscapedPath() [+ "?comp=<v>" if present]`. Only the `?comp=` query parameter is signed. The verifier now accepts both schemes; existing `SharedKey` storage-azureblob conformance still passes. Single-byte change to the dispatch + a new `buildTablesCanonicalString` helper.
- **Conformance — 3 SDK tests, all green.** `TestAzureSDK_CosmosTables_TableLifecycle` (Create / List / 409-on-duplicate / Delete), `TestAzureSDK_CosmosTables_EntityCRUD` (Insert / Get / List / Delete with `Edm.Int64` + `bool` + `string` types round-trip), `TestAzureSDK_CosmosTables_QueryByPartition` (3 entities, filter to 2 by partition key). CLI + Terraform conformance for Cosmos Tables goes through azurerm's ARM-managed `azurerm_cosmosdb_table` which needs ARM passthrough plumbing — deferred to the cross-cloud Apply matrix PR (same shape as the Azure DNS through-shim Apply work in PR #84-86).

Follow-on PRs: sockerless backend lanes (AWS + GCP sims); etcd K8s peer; cross-cloud Apply matrix (likely multiple PRs including azurerm ARM passthrough for the Terraform leg).

### 15.C GCP Firestore — backend + frontend + conformance (PR #92, after PR #91)

PR #91 closed the AWS DynamoDB slot. This PR closes the GCP slot. Firestore was called out as the trickiest cloud in PR #90's N18 because Firestore has no native tables — the shim manufactures one via the `__shim_tables__` reserved collection. This PR proves N18's design works end-to-end and surfaces one Firestore-specific trade-off that's now documented.

- **Spec ingest.** `services/nosql/spec/gcp-firestore-discovery.json` from `firestore.googleapis.com/$discovery/rest?version=v1` (revision `20260529`). `cmd/gcp-codegen` emits routing-only output at `services/nosql/gen/gcp/gcp_firestore.gen.go` (56 routes — the full Firestore Discovery surface). The frontend imports it with a blank import so spec drift fails the build.
- **Backend (`services/nosql/backends/gcp/gcp.go`).** Passthrough via `google.golang.org/api/firestore/v1`. Per-table metadata writes to `projects/{p}/databases/{db}/documents/__shim_tables__/{name}` (a stringValue + mapValue document). Item docs live under `projects/{p}/databases/{db}/documents/{tableName}/{base64-url(<encoded-composite-key>)}`. The stateless rule survives — the metadata read happens per-request on PutItem/GetItem/DeleteItem to resolve the schema; no shim-side cache.
- **The one Firestore-specific trade-off (carry-forward for N18).** Firestore's REST `:runQuery` endpoint streams `[RunQueryResponse, RunQueryResponse, ...]` over chunked transfer, but the Go `google.golang.org/api/firestore/v1` SDK's generated decoder reads only the FIRST element — the body type the SDK expects is `*RunQueryResponse` (single object). Calling `RunQuery(...).Do()` against a multi-document result gets `json: cannot unmarshal array into Go value of type RunQueryResponse`. The backend works around this by **using `documents.list` + client-side filter for the Query path** instead of `:runQuery`. This is O(N) on collection size and acceptable for the fidelity-first stance; the shim doesn't claim push-down query performance. Documented at the Query method.
- **Frontend (`internal/nosql/frontends/gcp_firestore/server.go`).** Path-shape dispatch (same pattern as `internal/dns/frontends/gcp_clouddns/`). Writes/reads/deletes on `__shim_tables__` go to `domain.{Create,Get,Delete,List}Table` and `UpdateTableTags`; everything else routes to `domain.{Put,Get,Delete}Item` / `Scan` / `Query`. The shim's `:runQuery` handler still emits the array-shape response real Firestore uses — raw-HTTP and gRPC callers see every document; only the Go REST SDK's `.Do()` is limited.
- **DocID encoding.** Firestore identifies docs by docID under a collection; the shim must round-trip a domain.Key (PartitionKey + optional SortKey) through a single string. Backend uses base64-url(typed-prefix-segments-joined-by-pipe); frontend uses a parallel `derivedDocID` over the item attributes so the docID is deterministic from the user's PutItem payload. The encodings differ slightly across backend (composite key only) vs frontend (every attribute); they don't need to match because the docID is opaque to the user — the backend reverses it to a key when called from a frontend, and the frontend re-derives it when echoing back to the SDK.
- **Conformance — SDK (4 tests) + raw HTTP (1 test) — all green.** `TestGCPSDK_Firestore_TableLifecycle` (CreateTable / Get / List / Delete via __shim_tables__), `TestGCPSDK_Firestore_ItemCRUD` (every value type round-trip), `TestGCPSDK_Firestore_ListDocuments` (Scan path via documents.list), `TestGCPHTTP_Firestore_RunQueryRaw` (the array-shape runQuery response, validating partition-key match + sort-key range filter). No CLI test (gcloud firestore has no per-document surface — database-level only). Terraform deferred to the cross-cloud Apply PR.

Follow-on PRs: Azure Cosmos Tables backend + frontend; sockerless backend lane (AWS + GCP sims); etcd K8s peer; cross-cloud Apply matrix.

### 15.C AWS DynamoDB — backend + frontend + conformance (PR #91, after PR #90)

PR #90 landed the NoSQL domain interface + in-memory backend + N18 / N19. This PR closes the first cloud slot: AWS DynamoDB. The shape mirrors the DNS 15.D series — one cloud at a time, each PR carries its own backend + frontend + SDK/CLI/Terraform conformance.

- **Spec ingest.** `services/nosql/spec/aws-dynamodb.smithy.json` vendored from `aws-sdk-go-v2/codegen/sdk-codegen/aws-models/dynamodb.json` at SHA `13bee3c72e77d68d6180fb633a7ce1c8714d53c0`. `cmd/codegen` emits `services/nosql/gen/aws_dynamodb.gen.go` with 14 operations: CreateTable / DeleteTable / DescribeTable / ListTables / PutItem / GetItem / DeleteItem / Scan / Query / TagResource / UntagResource / ListTagsOfResource / DescribeTimeToLive / DescribeContinuousBackups.
- **Why DescribeTimeToLive / DescribeContinuousBackups even though TTL + PITR are out of intersection.** `hashicorp/aws`'s `aws_dynamodb_table` Read path calls both after every Apply to populate computed fields. Not registering the route would surface as `UnknownOperationException` and break Terraform conformance. The handlers return `DISABLED` for every table — honest answer for a shim that doesn't implement TTL or PITR; the alternative ("fail loudly") would be unusable for any Terraform workflow.
- **Backend (`services/nosql/backends/aws/aws.go`).** Passthrough via `aws-sdk-go-v2/service/dynamodb`. Per-request `DescribeTable` resolves partition / sort key names for item ops — no shim-side schema cache, per the stateless rule. Pagination tokens encoded as `<name>=<typed-value>` strings (s/n/b/x/_ prefixes) for round-trip via `domain.{Scan,Query}Options.PageToken`. Tag round-trip via `UpdateTableTags` → `Untag` + `Tag` against the real DynamoDB ARN. Composite types (L / M / SS / NS / BS) reject at the boundary; the value translation in `valueFromAWS` covers the in-intersection scalars from N19.
- **Frontend (`internal/nosql/frontends/aws_dynamodb/adapter.go`).** Implements the generated `gen.DynamoDBBackend` interface by translating each per-op input → `domain.NoSQL` call → output. SigV4 verifier middleware on the way in (same `AKIAIOSFODNN7EXAMPLE` test credentials as every other AWS frontend). `KeyConditionExpression` parser supports the intersection subset: `pk = :v [AND begins_with(sk, :p)]`; complex grammar (BETWEEN, <, >, etc.) rejects with `ValidationException` rather than fabricating success — the honest "not in intersection" surface that lets cross-cloud Apply remain deterministic when GCP Firestore + Azure Cosmos Tables frontends land.
- **`domain.NoSQL` gains `UpdateTableTags`.** Same pattern as `domain.Secrets.UpdateSecret` (tags map replacement). Allows `TagResource` + `UntagResource` to read-modify-write through the domain rather than mutating tags in the frontend (which would violate the "frontend is wire-translation only" rule).
- **Conformance — SDK + CLI + Terraform — all green against the in-memory backend.** Six test functions in `services/nosql/conformance/`: `TestAWSSDK_DynamoDB_TableLifecycle`, `TestAWSSDK_DynamoDB_ItemCRUD_PartitionOnly` (every value type), `TestAWSSDK_DynamoDB_QueryAndScan_CompositeKey`, `TestAWSSDK_DynamoDB_TagsRoundTrip`, `TestAWSCLI_DynamoDB_TableAndItem`, `TestTerraform_AWSDynamoDB_TableLifecycle` (init + apply + destroy clean).

Follow-on PRs: sockerless backend lane (drive the AWS backend against the sockerless AWS sim's DynamoDB handler); GCP Firestore backend + frontend with `__shim_tables__` per N18; Azure Cosmos Tables backend + frontend; etcd K8s peer; cross-cloud Apply matrix.

### 15.C NoSQL foundational — domain + inmem + N18 / N19 (PR #90, after PR #89)
