...
**Follow-on cells: Service Bus Apply (this PR, after PR #59) — paused on [sockerless#276](https://github.com/e6qu/sockerless/issues/276).** Adding `azurerm_servicebus_namespace` + `azurerm_servicebus_queue` Apply was the third 14.E expansion. The shape differs from storage/KV: shimanism's `internal/queue/frontends/azure_servicebus` is REST/ATOM-only by design — AMQP listening lives in sockerless, not the shim. So the shim's *frontend* isn't on the data-plane path here; the through-shim coverage is on the *backend* translation layer. The shim's `azurequeue` backend parses the SAS connection string azurerm emits and dispatches via `azservicebus.Client` with `CustomEndpoint` pinned at sockerless's AMQP port. Test reads the namespace's `default_primary_connection_string` as a Terraform output and feeds it straight into the backend's config — no hand-built connection strings, no hardcoded SAS keys.

CI surfaced a sockerless gap that paused the cell: azurerm reads `Microsoft.ServiceBus/namespaces/{name}/networkRuleSets/default` after namespace creation to populate the `network_rule_set` computed attribute, and sockerless's SB ARM surface doesn't implement that sub-resource. Filed sockerless#276 with a suggested response body (Azure's "Allow / public-enabled" default works) and noted two other sub-resources (`disasterRecoveryConfigs` list, `migrationConfigurations/$default`) likely to need the same fix. The cell stays in tree as `t.Skip("blocked on sockerless#276")` so the wiring is preserved for one-line re-enable.

The clarifying realisation that survives the pause: not every shimmed service has both frontend-translation AND backend-translation through-shim coverage; for SB the meaningful through-shim test exercises the backend layer.
