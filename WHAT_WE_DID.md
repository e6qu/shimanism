...
- **Terraform conformance for `azurerm_cosmosdb_table` is deferred.** azurerm drives the resource through ARM at `Microsoft.DocumentDB/databaseAccounts/{a}/tables/{n}`. Sockerless's Cosmos handler simulates Core SQL (`databaseAccounts/{a}/sqlDatabases/...`) but not Tables ARM. With user approval, an upstream issue against sockerless will request Tables ARM coverage; once that lands, the TF test follows.
