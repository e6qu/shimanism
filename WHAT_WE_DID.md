...
- **12.A.26/27/29/30** pin BUG-20 with realistic-shape regression tests for the three ARM types. Decode + JSON round-trip a Create body through `gen.ContainerApp`, `gen.RedisResource`, and `gen.Server` (PostgreSQL FlexibleServer); assert the schema's own properties (not just the inherited TrackedResource fields) decode + re-encode cleanly. A future preprocessor regression that re-aliased these types to TrackedResource or dropped JSON tags would trip these tests in <100ms instead of surfacing as adapter-migration churn.
- **12.A.31** discovered + fixed a chained-inheritance bug in `flattenARMAllOf`. The direct unit test (`X → allOf [Y]; Y → allOf [Z]`) failed because the implementation cleared the allOf array on its first merge, so the second pass had nothing to iterate. Fix: defer merging from a source that still has unresolved allOf of its own; the walker iterates until no definition changes; pass N pulls from sources resolved in pass N-1. Real-spec impact: cache/Redis ARM, functions/ContainerApps, rdbms/PostgreSQL FlexibleServers gen files all grew with previously-missing inherited fields.
- **12.A.32/33/34/35** round out direct unit coverage to every substantive Azure preprocessor stage: `flattenXMSPaths` (basic move + paths-win-on-conflict + noop), `dedupeParameterDefNameCollisions` (stamps `x-go-name`, respects authored value), `promoteXMsEnumName` (promotes inline schema to `$ref`, skips parameters/headers), `classifyRef` (every ref shape), `normalizeAllOf` (nils empty slice, preserves populated).
- **12.A.36** caught a real Makefile bug via `TestGenHeadersCarryProvenance`: `services/pubsub/gen/azure/azure_servicebus.gen.go` shipped with a placeholder zero-SHA in its header. Root cause: pubsub's `azure-codegen.json` points at `services/queue/spec/azure-servicebus.json` (Service Bus is a shared spec — one vendored file drives both queue + pubsub gen). The Makefile's commit-extract was greping the MANIFEST's service dir, not the SPEC's directory — so for shared-spec services the SOURCES.md lookup found no row and fell back to the placeholder. Fix: extract commit from `$spec_dir/SOURCES.md` instead. Pubsub gen now carries the real SHA.
- **12.B.7/8/9** ship the GCP routing emitter's runtime helpers: each Route now carries a compiled `*regexp.Regexp` + `Vars []string` derived from the Discovery URI template; `Match(method, path)` returns the first candidate, `MatchAll(method, path)` returns every candidate (needed for Discovery services like Secret Manager + Pub/Sub that overload `v1/{+name}` across many ops). Per-service dispatch-coverage tests assert each frontend's sample request shapes resolve to the expected op ID via the gen inventory.
- **12.B.14** per-service `BasePath` sanity test. Storage's Discovery declares `/storage/v1/`; the other 7 declare empty. A Discovery field-shape change or emitter regression that miscaptures basePath fails before downstream `Match()` consumers see the wrong full-URL anchor.
- **12.B.17/18** every-route round-trip test across all 8 GCP services: for each Route, synthesize a sample path by substituting URI-template variables, then assert Pattern matches. 413 routes covered. Catches the regression class where `templateToRegex` emits a pattern that fails to match its own template-derived path.
- **12.0.1/2** spec-freshness lane: `make spec-freshness` + a weekly GitHub workflow (Mondays 14:00 UTC) walk every services/*/spec/SOURCES.md, query the upstream HEAD on the pinned path, and report `ok`/`DRIFT`/`skip` per spec. Renovate doesn't track vendored specs; this fills the gap.
- **12.0.6** Azure + GCP fetch scripts mirror the AWS pattern. All three call `cmd/inject-provenance` after download so the spec's `_provenance` stays current.
- **12.0.11** `TestProvenanceMatchesSOURCES` walks each SOURCES.md and asserts every row matches the corresponding spec file's `_provenance` block. Catches the case where a contributor edits SOURCES.md but forgets `make inject-provenance`.
- **12.0.12/13** per-service Terraform walkthrough in every MIGRATION.md (HCL pulled from each service's `cross_cloud_import_test.go` fixture so the example is real, not invented). Closes Phase 12 exit criterion 3.
- **12.0.15** Renovate custom manager tracks vendored-spec SHAs. Renovate can't refresh the vendored file (verbatim copy requires `scripts/fetch-*.sh` to re-inject `_provenance`), but it CAN watch upstream HEAD for new commits on the pinned path and open an issue when the SHA falls behind.
- **12.0.16** Phase 13 stub in PLAN.md frames the follow-on (full adapter migration, production RS256 JWKS, real-cloud Track A).
- **12.0.17–22** doc lattice refresh: docs/codegen.md, docs/architecture.md, docs/development.md all reflect three-lane reality. Broken `internal/operations/` ref in rdbms/MIGRATION.md fixed. `make help` lists every target grouped by purpose.
- **12.0.24** codegen-check also runs inject-provenance, so a SOURCES.md edit without re-injection trips the pre-push lane (not just CI).
- **12.9** Phase 12 closer flipped from ◐ to ✅. All three exit criteria met.
