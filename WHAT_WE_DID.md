...
PR #62 added `TestSockerless_E2E_AzureBlob_Through_Shim_ApplyTF_BackendAWS`: same azurerm storage account + container Apply as PR #58, but the shim's azure_blob backend is the AWS S3 backend talking to sockerless's AWS sim — verifies the "container" the Azure-shaped Terraform created lands as an S3 bucket in the AWS-shaped store. This PR mirrors that on the GCP corner: `TestSockerless_E2E_AzureBlob_Through_Shim_ApplyTF_BackendGCS` runs the same Apply against sockerless's GCP sim via the shim's GCS backend, verifying the bucket lands in GCS. Together, the two cells close the cross-cloud Apply matrix on Azure storage's two destination corners (AWS and GCP). End-to-end this is the cross-cloud Apply promise: write Azure-shape Terraform, get an AWS- or GCP-shape resource on the destination cloud.
