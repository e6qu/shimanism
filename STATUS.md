...
| In-flight | **14.E KV through-shim Apply.** All sockerless gates landed: #259 endpoint emission (covers `keyVault` template + `{vault}` interpolation), #262 RS256 JWKS-published Azure AD tokens, #271 `{account}` interpolation + auto-derived `storage` suffix, **#274 per-resource `aud` from OAuth `scope`/`resource` (closed #272)**. `scripts/run-sockerless-storage.sh` emits `keyVault:"https://{vault}.vault.localhost:14582/"` alongside the storage template; the self-signed cert SAN now covers `*.vault.localhost`. `harness.StartSecretsServerAzureKVAtPort` binds the shim's KV frontend with TLS using the same cert sockerless does and an `azurebearer` verifier whose JWKS is pre-fetched out-of-band from sockerless's `/{tenant}/discovery/v2.0/keys`. New `TestSockerless_E2E_AzureKV_Through_Shim_ApplyTF` drives `azurerm_key_vault` + `azurerm_key_vault_access_policy` + `azurerm_key_vault_secret` end-to-end against the inmem secrets backend. Linux-only (SSL_CERT_FILE platform limit). Track A blocked on real-cloud credentials. |
