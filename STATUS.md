...
| | |
|---|---|
| Active branch | `phase-11` (PR #18 draft). 11.0 / 11.1 / 11.2 / 11.3a / 11.3b landed; next is 11.3c (wire secrets adapter + delete hand-written wire). |
| In-flight | **Phase 11 — Tighten the wire boundary.** Spec-driven codegen across every service + signature verification (BUG-18) at the new decode boundary. 11.3b closed: `awsjson.EpochTime` runtime type + emitter epoch-seconds timestamp awareness — fixes the awsJson1_x SDK-wire-compatibility for timestamp fields (Go's RFC3339 default broke it). 11.3a closed: `make codegen` generalised across all `services/*/codegen.json` manifests; secrets and queue gen files in tree (11 + 9 ops). Next: 11.3c writes the per-op adapter implementing `gen.SecretsManagerBackend`, wires it into the harness, deletes the 691-line hand-written wire layer. Full sub-task table in [PLAN.md § Phase 11](PLAN.md#phase-11--tighten-the-wire-boundary). |
