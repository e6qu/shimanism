...
| | |
|---|---|
| Active branch | `main` after PR #79 merged (15.D foundational). Phase 15.D AWS Route 53 backend PR in flight. |
| In-flight | **15.D AWS Route 53 backend.** Vendored `aws-route-53.smithy.json` (Smithy, rest-xml protocol; `aws/aws-sdk-go-v2`@`13bee3c72`); `services/dns/codegen.json` manifests 8 intersection ops (CreateHostedZone / DeleteHostedZone / GetHostedZone / ListHostedZones / ChangeResourceRecordSets / ListResourceRecordSets / ChangeTagsForResource / ListTagsForResource); `make codegen` emits `services/dns/gen/aws_route53.gen.go` (~39 KB). `services/dns/backends/aws/aws.go` implements `domain.DNS` against `aws-sdk-go-v2/service/route53` — name→HostedZoneId resolved per request via `ListHostedZonesByName` (stateless), N17 dispatch on `Visibility`, TXT double-quoting on write + strip on read, `DeleteZone(force=true)` enumerates + batches DELETE skipping cloud-managed SOA + apex NS. Pure-Go helper tests in `aws_test.go`. Backend-only PR; frontend + SDK / CLI / Terraform conformance lands in the next PR. Track A blocked on real-cloud credentials. |
| Last merged | PR #79 — 15.D foundational (domain + inmem + N17). |
