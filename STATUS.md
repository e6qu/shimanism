...
| | |
|---|---|
| Active branch | `15c-gcp-firestore-backend-frontend` — 15.C NoSQL GCP Firestore Native backend + frontend + SDK + raw-HTTP conformance. |
| In-flight | **15.C GCP Firestore — backend + frontend + conformance.** Vendored Firestore Discovery doc (`services/nosql/spec/gcp-firestore-discovery.json`, revision 20260529); codegen emits the routing table at `services/nosql/gen/gcp/`. Backend at `services/nosql/backends/gcp/gcp.go` implements N18's `__shim_tables__` metadata-collection convention (the one stateless-shim deviation Firestore forces — the metadata write lives in Firestore itself). Frontend at `internal/nosql/frontends/gcp_firestore/` dispatches Firestore REST API → `domain.NoSQL`. Query falls back to documents.list + client-side filter (the Go SDK's RunQuery decoder can't handle Firestore's streaming-array response; documented). Conformance covers SDK item CRUD + table lifecycle + ListDocuments, plus raw-HTTP runQuery against the array-shape response. No CLI test (gcloud firestore has no per-document surface); Terraform deferred to cross-cloud Apply PR. |
