...
| In-flight | **Phase 12 — Cross-cloud cell expansion + Phase 11 follow-ons.** PR #19 carries both wire-boundary tracks. Track 2.A: `azure_keyvault` fully spec-driven (12.A.1/2); `cmd/azure-codegen` validated across 5 Azure specs (KV + SB data-plane + Cache + APIM + ContainerApps ARM); common-types inliner with v1–v6 vendored + cross-version + multi-file sibling refs; gen/azure subpackage. Track 2.B: `cmd/gcp-codegen` Discovery JSON → routing-only `gen.gcp.Routes` inventory; vendored Discovery for all 8 GCP services. Three pipelines now flow through `make codegen` (Smithy → Azure → GCP). Adapter migrations (12.A.11+ Azure, 12.B.2+ GCP) are mechanical follow-ons. PostgreSQL (duplicate typename) + Azure Blob (x-ms-paths) parked pending per-spec preprocessing. |
