...
| | |
|---|---|
| Active branch | `phase-13` (created from `main` 2026-05-22 after PR #19 merged). Next branch will be `phase-14` once PR #20 merges. |
| In-flight | **Phase 13 — Full adapter migration + production auth + real-cloud Track A.** All 7 Azure + 8 GCP frontends now sit on the gen-inventory spec-drift contract on PR #20. **Full handler migrations** (Server implements gen.ServerInterface; handlers dispatch through the gen path): `azure_redis` (13.A.1), `azure_containerapps` (13.A.2), `azure_dbadmin` (13.A.3), `azure_servicebus` queue (13.A.4), `azure_servicebus_topics` pubsub (13.A.5), `gcp_secretmanager` (13.B.1). 13.A.4/5 use a hand-written-dispatch hybrid because Go 1.22's ServeMux refuses the Service Bus admin spec's overlapping patterns. **Spec-drift contract via blank import** (build-time gate; full handler migration follow-on): `azure_blob` (13.A.6), `azure_apim` (13.A.7), `gcp_apigateway` / `gcp_memorystore` / `gcp_cloudrun` / `gcp_pubsub` × 2 / `gcp_cloudsql` / `gcs` (13.B.2-8). 13.C production RS256 JWKS landed in `internal/{gcpbearer,azurebearer}/jwks.go` — both verifiers now branch on JWT header `alg`: HS256 (test mode, existing) vs RS256 (JWKS lookup → RSA pubkey reconstruction → `crypto/rsa.VerifyPKCS1v15`). Config via `Options.JWKSURL` (URL-fetched + cached + kid-rotation re-fetch) or `Options.JWKS` (in-process). Unit tests cover both paths + signature tampering + claims rejection. **13.D.1 sockerless validation lane** — opt-in `make sockerless-storage` target wired; AWS S3 bucket lifecycle + GCS full round-trip + AWS Secrets Manager CreateSecret/ListSecrets/DeleteSecret pass against the local simulators (see [doc/SOCKERLESS_VALIDATION.md](doc/SOCKERLESS_VALIDATION.md)). Six upstream issues filed against sockerless: 3 fidelity bugs ([#173](https://github.com/e6qu/sockerless/issues/173) S3 URL prefix, [#174](https://github.com/e6qu/sockerless/issues/174) aws-chunked envelope, [#175](https://github.com/e6qu/sockerless/issues/175) missing ListSecretVersionIds) + 3 missing-service rollups per cloud ([#176](https://github.com/e6qu/sockerless/issues/176) AWS SQS/SNS/APIGW/RDS/ElastiCache, [#177](https://github.com/e6qu/sockerless/issues/177) GCP Pub/Sub/Secrets/SQL/Memorystore/APIGW, [#178](https://github.com/e6qu/sockerless/issues/178) Azure Blob+KeyVault data plane / Service Bus / PG / Redis / APIM). **All Phase-13-deferred items now belong to Phase 14**, gated on sockerless issue resolution — see [PLAN.md § Phase 14](PLAN.md#phase-14--sockerless-verified-validation-lane--deferred-follow-ons). Remaining for next Phase 13 PR: 13.D Track A real-cloud walk (still requires real AWS accounts; closes BUG-8 + reclassifies BUG-15). |
