...
| In-flight | **Phase 1.4 → 1.13 stacked on PR #6.** 1.4: intersection scoping + full conformance harness (SDK + CLI + Terraform). 1.5.0: neutral `domain.Storage` + AWS frontend adapter + streaming codegen. 1.5.1: MinIO. 1.5.2: AWS passthrough. 1.6: GCS (cross-shape; multipart via temp-objects + compose). 1.7: Azure Blob (native block list via base64 block IDs). 1.8: K8s peer — runnable `shim storage` binary + `deploy/k8s/peer/` manifests + Dockerfile. 1.9: Azure CopyObject fail-loud poll loop. 1.10: canonical S3 multipart ETag via `domain.MultipartETag`. 1.11: presigned URL conformance test. 1.12: BUG-1 fix via `restxml.RouteOptions.ForbiddenQueries` + object-level probes (GetObjectTagging, GetObjectAcl). 1.13: CI conformance matrix (minio / gcs / azureblob lanes). **Phase 1 is not yet done** — sub-phases 1.14 (GCS frontend) + 1.15 (Azure Blob frontend) + 1.16 (full-matrix closer) are still pending per PLAN.md principle 11 (every phase ships every frontend × every backend). |
