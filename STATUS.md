...
| In-flight | **15.B N13 + SB cross-cloud (final two 15.B audit items).** N13: keep `domain.cache.NodeType` as opaque pass-through; the ergonomic gain of a normalised small/medium/large enum doesn't yet justify maintaining three per-cloud mapping tables. N16 (new): SB cross-cloud Apply via a shim AMQP frontend is **deliberately out-of-intersection** — building an AMQP listener at the shim is multi-PR Phase-16+ work; the supported SB Apply path is backend-translation-layer (PRs #60 / #61). Documented in `docs/normalizations.md` (§ N13 closing note + new § N16) and `services/queue/APPLY_INTERSECTION.md`. Track A blocked on real-cloud credentials. |
