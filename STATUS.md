...
| | |
|---|---|
| Active branch | `main` after PR #54 merged 2026-05-28 (mock-Microsoft-Entra + first through-shim azurerm Terraform Apply for storage). Phase 14.E.5 in flight: extend the same pattern to Key Vault. |
| In-flight | **14.E.5 — Through-shim azurerm Terraform Apply for Key Vault.** Mirrors PR #54's storage pattern: KV ARM frontend gains `Options{VaultURI, TrackVaults}` (parallel to the storage `Options{BlobEndpoint, TrackAccounts}`); new `TestCrossCloudApply_Roundtrip_KeyVaultAzureToAWS` drives `azurerm_key_vault` + `azurerm_key_vault_secret` end-to-end through mock-AAD + ARM + data-plane shims, verifying `shim-applied-secret` lands in the backend. Linux-only (SSL_CERT_FILE). The previously-skipped `TestTerraform_AzureSecrets_ResourceLifecycle` stays as a redirect stub. `make sockerless` unchanged at 45 passing. After this, 6 more services (cache, queue, pubsub, functions, rdbms, apigateway) follow the same pattern. Track A blocked on real-cloud credentials. |
| Last merged | PR #54 — 14.E.4: mock Microsoft Entra + first through-shim azurerm Terraform Apply (storage), 2026-05-28. |
