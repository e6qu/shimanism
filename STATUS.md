...
| | |
|---|---|
| Active branch | `phase-11` (PR #18 draft). Major progress: 11.0–11.3 / 11.6 (all sub-phases) / 11.7a / 11.8a / 11.9 / 11.10 / 11.11a / 11.12a / 11.13 (all sub-phases) all landed. 4 AWS frontends spec-driven and SigV4-protected; 24/24 frontends across all 3 clouds verifier-wrapped. Remaining: pubsub/rdbms/cache AWS adapter migrations (gen files in tree, awsQuery emitter ready); GCP routing emitter + adapter migrations; Azure oapi-codegen pilot + adapter migrations; conformance lane rewrite to sign with the test key; closer. |
| In-flight | **Phase 11 — Tighten the wire boundary.** Spec-driven codegen + signature verification (BUG-18). Landed this PR: 4 emitter protocol paths (REST-XML, awsJson1_x, restJson1, awsQuery); generated stubs in tree for all 8 services' AWS surfaces; 4 AWS frontends migrated to spec-driven (secrets + queue + lambda + apigateway, 2527 LOC of hand-written wire deleted); 4 verifier packages with Middleware() (sigv4verifier, gcpbearer, azurebearer, azuresharedkey) — 23 unit tests + 3 end-to-end SigV4 reject conformance tests; 24/24 frontends verifier-wrapped in the harness; BUG-18 P1 → P3. `SHIMANISM_TEST_UNAUTHENTICATED=1` bypass set by harness init() keeps existing conformance lanes green during the test rewrite. Honest remaining-work in [PLAN.md § Phase 11 § Remaining work](PLAN.md#remaining-work-honest). |
