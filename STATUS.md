...
| | |
|---|---|
| Active branch | `main` after PR #74 merged (15.A audit closed: N1–N15). Phase 15.B residual-cleanups PR in flight: documents the terraform-aws `has_secret_string_wo` drift investigation. |
| In-flight | **15.B `_wo` drift investigation.** terraform-aws v5.100+ `aws_secretsmanager_secret_version` schema adds `has_secret_string_wo` (computed bool) that the provider's Read doesn't populate when the resource is created via regular `secret_string` path. Terraform shows `(known after apply) # forces replacement` on every plan-after-apply. `lifecycle.ignore_changes` doesn't help (provider-only computed). Shim's translation rule N1 is innocent — sockerless variants pass. Re-skip the in-process test with the clear rationale; document the investigation in `docs/normalizations.md` under "Phase 15.B investigation". Filing upstream at `hashicorp/terraform-provider-aws` pending user approval per AGENTS rule. Track A blocked on real-cloud credentials. |
| Last merged | PR #74 — 15.A N15: API Gateway declarative-replace routing table (first-pass audit closed). |
| Upstream watch | sockerless #288 merged (AWS API Gateway client-surface coverage) — useful context for future 15.D DNS scoping work. Zero open sockerless issues for shimanism's current work. |
