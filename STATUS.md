...
| In-flight | **Phase 1.4 → 1.16 stacked on PR #6, near-complete.** Three real frontends (AWS S3 / GCS / Azure Blob) × five backends (inmem / minio / aws / gcs / azureblob) wired through `domain.Storage`. Conformance: TestConformanceMatrix_{AWSFrontend,GCSFrontend,AzureBlobFrontend} drives each frontend's official Go SDK across every backend. CLI rows: aws + gcloud (bucket lifecycle, gcloud object cp blocked on upstream gcloud bug — SDK covers) + az (runs when on PATH). Terraform rows: hashicorp/aws (resource lifecycle) + hashicorp/google (resource lifecycle) + hashicorp/azurerm (skipped, no provider-level blob endpoint override). 1.13 CI matrix in place (minio / gcs / azureblob lanes). Waiting on CI green before declaring 1.16 done. |
