...
| | |
|---|---|
| Active branch | `phase-11` (PR #18 draft). Substantial progress: 11.0 / 11.1 / 11.2 / 11.3 / 11.7a / 11.9a / 11.10a / 11.6a all landed. Remaining: awsQuery emitter, Lambda+APIGW adapter swaps, per-frontend SigV4 wiring, GCP routing + Azure oapi-codegen + per-service adapters, storage retrofit, closer. |
| In-flight | **Phase 11 — Tighten the wire boundary.** Spec-driven codegen + signature verification (BUG-18). Landed this PR: `internal/awsjson/` runtime helper (with EpochTime + QueryCompatibleCode); 3 protocol templates in the emitter (REST-XML pre-existing, awsJson1_x new, restJson1 new); generated stubs for storage (36 ops, REST-XML), secrets (11 ops, awsJson1_1), queue (14 ops, awsJson1_0), functions (14 ops, restJson1), apigateway (12 ops, restJson1); secrets + queue AWS frontends fully migrated to spec-driven (1544 LOC of hand-written wire deleted); `internal/sigv4verifier/` package (re-uses `signer/v4` building blocks, 7 unit tests covering accept/reject/tampered/stale/restore). Honest remaining-work in [PLAN.md § Phase 11 § Remaining work](PLAN.md#remaining-work-honest). |
