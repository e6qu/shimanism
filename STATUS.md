...
| In-flight | **Phase 12 — Cross-cloud cell expansion + Phase 11 follow-ons.** Track 2.A progress on PR #19: `azure_keyvault` fully spec-driven (12.A.1/2); `cmd/azure-codegen` toolchain validated across 5 Azure specs (KV + SB data-plane + Cache + APIM + ContainerApps ARM); common-types inliner with v1–v6 vendored + cross-version refs; multi-file sibling inliner for spec-local refs; rewrite-merged-content fix prevents context-mixing on shorthand refs; gen/azure subpackage convention to avoid AWS-vs-Azure type collisions. PostgreSQL (duplicate typename) + Azure Blob (x-ms-paths) parked pending per-spec preprocessing. Track 1 (cross-cloud cells) largely covered by Phase 10. |
