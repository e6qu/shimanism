...
| | |
|---|---|
| Active branch | `phase-11` (PR #18 draft, ready). **Phase 11 closed: BUG-18 closed.** 8/8 AWS frontends spec-driven; 24/24 frontends verifier-wrapped; bypass dropped from harness init(); every conformance test signs end-to-end across all 3 clouds (AWS SigV4 + GCP HS256 JWT + Azure Bearer/SharedKey). |
| In-flight | **Phase 11 — Tighten the wire boundary — CLOSED.** Spec-driven codegen (4 emitter protocol paths: REST-XML, awsJson1_x, restJson1, awsQuery — 3853 LOC of hand-written wire deleted) + signature verification (4 verifier packages: `sigv4verifier`, `gcpbearer`, `azurebearer`, `azuresharedkey`). Manual SigV4 canonical-request implementation in `internal/sigv4verifier/canonical.go` accepts both Go-SDK and boto3 (`aws` CLI) signing shapes uniformly. Presigned-URL SigV4 verification path landed too. `azuresharedkey` uses `EscapedPath()` to match the azblob SDK's signed path. BUG-18 P1 → P3 → CLOSED. Two collateral defects surfaced + fixed during closure: awsQuery map-shape response fields silently dropped on XML marshal (added emitter-generated `MarshalXML` per Smithy map type); SNS GetTopicAttributes Policy empty + SetTopicAttributes rejecting AWS-only feedback-rate attributes terraform-provider-aws sends on every apply (canonical default policy + AWS-only allowlist no-op). |
