...
| | |
|---|---|
| Active branch | `phase-11` (PR #18 draft). Plan landed; codex review folded back; Phase 12 added for cross-cloud cell expansion. Code work opens at 11.1 (architecture spike). |
| In-flight | **Phase 11 — Tighten the wire boundary.** Spec-driven codegen across every service + signature verification (BUG-18) at the new decode boundary. Coupled per service. Codex review corrected several wrong-library premises in the initial draft (SigV4 is signer-only not verifier; `golang.org/x/oauth2` is token-acquisition not JWT verifier; Key Vault is Bearer-only not SharedKey; current Smithy emitter is REST-XML, extending to awsJson / awsQuery / restJson1 is new emitter work). Revised plan starts with an architecture spike (11.1) to lock in verifier libraries + GCP gRPC-vs-REST decision before code. Full sub-task table in [PLAN.md § Phase 11](PLAN.md#phase-11--tighten-the-wire-boundary). |
