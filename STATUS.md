...
| In-flight | **Phase 13 — Full adapter migration + production auth + real-cloud Track A.** 5/7 Azure full adapter migrations + 1/8 GCP full migration + 7/8 GCP spec-drift contract via blank-import landed on PR #20: `azure_redis` (13.A.1), `azure_containerapps` (13.A.2), `azure_dbadmin` (13.A.3), `azure_servicebus` queue (13.A.4), `azure_servicebus_topics` pubsub (13.A.5), `gcp_secretmanager` (13.B.1 full migration), `gcp_apigateway` / `gcp_memorystore` / `gcp_cloudrun` / `gcp_pubsub` × 2 / `gcp_cloudsql` / `gcs` (13.B.2-8 spec-drift blank-import). 13.A.4/5 use a hand-written-dispatch hybrid because Go 1.22's ServeMux refuses the Service Bus admin spec's overlapping patterns. Remaining: 13.A.6 (azure_blob — 69-method data-plane spec with query-discriminated URLs), 13.A.7 (azure_apim — empty spec, deferable), 13.C (RS256 JWKS), 13.D (Track A). |
