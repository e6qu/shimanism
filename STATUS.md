...
| Bugs | 19 filed · 13 fixed · 5 open · 1 false positive. Phase 10 to-date: filed BUG-14 (false positive), BUG-15, BUG-16, BUG-17; closed BUG-2, BUG-13, BUG-16, BUG-17 in 10.3. Post-codex-review pass: filed BUG-18 (P1: no signature validation across frontends) + BUG-19; closed BUG-3 + BUG-4 (stale entries — fixes were already in tree) + BUG-19 (removed the stale Phase-3 TF skip) + BUG-12 (queue tag storage as domain primitive + 5 backends + AWS frontend dispatch + `TestTerraform_AWSQueue_Apply_NoDrift` extended with `tags = {…}` round-trip). Remaining open: BUG-6 (Azure APIM v3 delete), BUG-7 (`az` CLI override gap — `az` limitation), BUG-8 (`hashicorp/google` API Gateway OAuth — Track A), BUG-15 (GCP queue retention plan/apply asymmetry — partial fix), BUG-18 (signature verification across frontends — large fan-out, separate PR). |
| Live infra | None. |
