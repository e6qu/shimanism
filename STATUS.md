...
| | |
|---|---|
| Active branch | `main` after PR #82 merged (15.D GCP Cloud DNS). Phase 15.D Azure DNS + Private DNS PR in flight. |
| In-flight | **15.D Azure DNS + Private DNS frontend + backend + SDK conformance.** Vendored `azure-dns.json` (2018-05-01) + `azure-privatedns.json` (2024-06-01) from `Azure/azure-rest-api-specs`. **One backend** (`services/dns/backends/azure/azure.go`) dispatches on `domain.ZoneVisibility` per N17: public zones route through `armdns` (`Microsoft.Network/dnsZones`), private through `armprivatedns` (`Microsoft.Network/privateDnsZones`). **One frontend** (`internal/dns/frontends/azure_dns/server.go`) handles both ARM resource types via path-shape dispatch; wire types come from `armdns` + `armprivatedns` directly. TLS harness server (Azure SDK refuses Bearer over plain HTTP). SDK conformance passes; CLI/Terraform tests skipped (CLI: `az` requires custom-cloud config; TF: azurerm needs ARM resource-group + subscription ops the shim doesn't stub at this phase). Sockerless through-shim test skipped — wiring deferred to the cross-cloud Apply chunk. Track A blocked on real-cloud credentials. |
