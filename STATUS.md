...
| In-flight | **14.E Service Bus through-shim Apply.** sockerless#276 closed via [#277](https://github.com/e6qu/sockerless/pull/277) — namespace networkRuleSets/default GET/PUT/list, disaster recovery list, migration config 404 all landed (plus queue/topic/subscription ARM delete-response shape fixes). `azurerm_servicebus_namespace` + `azurerm_servicebus_queue` Apply via sockerless's real Microsoft.ServiceBus ARM; the namespace's `default_primary_connection_string` (a Terraform output) feeds the shim's azure_servicebus *backend*, which drives `azservicebus.Client` Send/Receive with `CustomEndpoint` pinned at sockerless's separate AMQP listener. Unlike storage / KV the shim's *frontend* isn't on the path — SB data plane is AMQP and shimanism's azure_servicebus frontend is REST/ATOM-only by design (sockerless owns the AMQP server). Through-shim coverage lives on the backend translation layer. Track A blocked on real-cloud credentials. |
