...
| In-flight | **15.C Cosmos Tables metadata + bearer (BUG-50 continued; PR #98 open).** Extends `internal/nosql/frontends/azure_cosmos_tables/Config` with `MetadataLoginURL` + `BearerOptions`. `HandlerWithConfig` splits auth by path: data-plane → SharedKey; ARM `/subscriptions/...` → bearer verifier → passthrough; `/metadata/endpoints` → in-band cloud-environment JSON (resourceManager = shim, login = sockerless). `internal/harness/server.go` gains `StartNoSQLServerAzureWithConfig`. Six unit tests cover ARM forwarding + metadata shape. **Terraform + az CLI conformance unblocked 2026-06-01**: [sockerless#356](https://github.com/e6qu/sockerless/issues/356) closed by [sockerless PR #357](https://github.com/e6qu/sockerless/pull/357) (Cosmos Tables ARM + Storage Tables ARM + `Prefer: return-no-content` / ACL). Next PRs (after #98 merges): `azurerm_cosmosdb_table` Terraform + `az cosmosdb table` CLI against the upgraded sockerless. |
| Last merged | PR #97 — BUG-50 Cosmos Tables ARM passthrough foundational (Config.Passthrough + unit tests + HTTPS harness). |
| Upstream watch | [sockerless#356](https://github.com/e6qu/sockerless/issues/356) closed by [sockerless PR #357](https://github.com/e6qu/sockerless/pull/357) on 2026-06-01 — Cosmos + Storage Tables ARM coverage. Unblocks `azurerm_cosmosdb_table` TF + `az cosmosdb table` CLI conformance (queued as next 15.C PRs). [sockerless PR #358](https://github.com/e6qu/sockerless/pull/358) merged 2026-06-02 — full Linux network namespace per VPC + NAT/IPAM across all three sims; fixes #336; advances #334 (ELBv2 real TCP/HTTP probes), #335 (EC2 SG nftables); #333 (Firecracker VM lifecycle) still open. Does not affect current Phase 15.C work. |
