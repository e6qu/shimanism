...
| In-flight | **Phase 12 — Cross-cloud cell expansion + Phase 11 follow-ons.** PR #19 carries both wire-boundary tracks. Track 2.A: `azure_keyvault` fully spec-driven (12.A.1/2); `cmd/azure-codegen` validated across **all 8 Azure specs** (KV + SB data-plane + Cache + APIM + ContainerApps + PostgreSQL ARM + pubsub shared SB + Blob data-plane); common-types inliner with v1–v6 vendored + cross-version + multi-file sibling refs; x-ms-enum inline-→-$ref preprocessor with parameter/header gating; parameter/definition name-collision deduper (`x-go-name: <N>Parameter`); deterministic codegen output; gen/azure subpackage; per-service azure_gen smoke tests with method-count guards (12.A.21/22). Track 2.B: `cmd/gcp-codegen` Discovery JSON → routing-only `gen.gcp.Routes` inventory with `(Route).Pattern` + `Match(method, path)` + `MatchAll(method, path)` helpers; vendored Discovery for all 8 GCP services; per-service frontend-dispatch-coverage tests (12.B.8/9). Spec-freshness check (12.0.1/2): `make spec-freshness` + weekly workflow surfaces drift between vendored specs and upstream HEADs. Three pipelines flow through `make codegen` (Smithy → Azure → GCP). Adapter migrations across the remaining 7 Azure + 7 GCP frontends are mechanical follow-ons (azure_keyvault is the reference impl). |
