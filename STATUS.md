...
| In-flight | **14.E Service Bus through-shim Apply — paused on [sockerless#276](https://github.com/e6qu/sockerless/issues/276).** Wiring is in place: `azurerm_servicebus_namespace` + `azurerm_servicebus_queue` apply via sockerless's real Microsoft.ServiceBus ARM; the namespace's `default_primary_connection_string` (a Terraform output) is fed into the shim's azure_servicebus *backend*, which drives `azservicebus.Client` Send/Receive with `CustomEndpoint` pinned at sockerless's separate AMQP listener. CI surfaced a sockerless gap: azurerm reads `namespaces/{name}/networkRuleSets/default` post-create to populate `network_rule_set`, and sockerless's SB surface doesn't implement that sub-resource yet (catch-all 404). The test is `t.Skip("blocked on sockerless#276")` pointing at the issue; re-enable once upstream lands. Track A blocked on real-cloud credentials. |
...
| Upstream | sockerless #257 / #260 / #261 / #269 / #272 all closed (via #259 / #262 / #271 / #274). **[#276](https://github.com/e6qu/sockerless/issues/276) open and blocking 14.E SB Apply** — `Microsoft.ServiceBus/namespaces/{name}/networkRuleSets/default` GET not implemented; azurerm's post-create read 404s. |
