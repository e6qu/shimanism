...
| Bugs | 19 filed · 14 fixed · 4 open · 1 false positive. Phase 10 to-date: filed BUG-14 (false positive), BUG-15, BUG-16, BUG-17; closed BUG-2, BUG-13, BUG-16, BUG-17 in 10.3. Post-codex-review pass: filed BUG-18 (P1: no signature validation across frontends) + BUG-19; closed BUG-3, BUG-4 (stale entries — fixes were already in tree), BUG-6 (Azure APIM v3 BeginDelete wired with ifMatch="*"), BUG-12 (queue tags as domain primitive + 5 backends), BUG-19 (removed stale Phase-3 TF skip). Remaining open: BUG-7 (`az` CLI override gap — `az` limitation, not a code bug), BUG-8 (`hashicorp/google` API Gateway OAuth — Track A), BUG-15 (GCP queue retention plan/apply asymmetry — partial fix), BUG-18 (signature verification across frontends — large fan-out, separate PR). |
| Live infra | None. |
