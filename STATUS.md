...
| | |
|---|---|
| Active branch | `main` after PR #53 merged 2026-05-28. Phase 14.E.4 in flight: mock-Microsoft-Entra endpoint + first through-shim `azurerm` Terraform Apply test. |
| In-flight | **14.E.4 — Mock Microsoft Entra.** New `internal/mockaad` package serves a minimal AAD-compatible OIDC token endpoint + cloud-metadata document over HTTPS. Used by the new `TestCrossCloudApply_Roundtrip_StorageAzureToAWS` cell to drive a real `hashicorp/azurerm` Terraform Apply against the shim's ARM + blob frontends, ending in an `applied-container` actually landing in the backend. Skips on macOS (SSL_CERT_FILE only works on Linux/Unix); CI Linux runners validate the full flow. This closes the LAST gap blocking through-shim azurerm Terraform Apply — the previously-skipped Azure Terraform conformance tests (storage, KV, cache, queue, pubsub, functions, rdbms, apigateway) can now follow the same pattern. `make sockerless` unchanged at 45 passing. Track A blocked on real-cloud credentials. |
| Last merged | PR #53 — 14.E.3: Storage ARM blob-endpoint propagation, 2026-05-28. |
