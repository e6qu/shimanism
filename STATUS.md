...
| In-flight | **14.E through-shim azurerm Apply (honest path) — paused on sockerless#269.** Wiring is in place: `scripts/run-sockerless-storage.sh` exports `SIM_AZURE_ARM_EXTERNAL_DATA_PLANE_URLS_JSON`, `harness.StartStorageServerAzureBlobAtPort` binds the shim's blob frontend with a SharedKey verifier seeded from sockerless's `simListKey64` derivation, and `TestSockerless_E2E_AzureBlob_Through_Shim_ApplyTF` drives a full azurerm Apply. CI surfaced a fourth sockerless gap: the literal `primary_blob_endpoint` emitted by #259 (e.g. `http://localhost:14581/`) is rejected by azurerm's parser, which requires `{account}.blob.{suffix}` shape and a matching `storage` suffix in `/metadata/endpoints`. Filed [sockerless#269](https://github.com/e6qu/sockerless/issues/269) for interpolated emission + suffix advertisement. The new test cell is currently `t.Skip("blocked on sockerless#269")`; re-enable once upstream lands. Track A blocked on real-cloud credentials. |
...
| Upstream | sockerless #257 / #260 / #261 all closed (via #259 / #262). **[#269](https://github.com/e6qu/sockerless/issues/269) open and blocking 14.E through-shim azurerm Apply** — current literal `primary_blob_endpoint` emission isn't azurerm-parseable; need `{account}.blob.{suffix}` interpolation + matching `storage` suffix in `/metadata/endpoints`. |
