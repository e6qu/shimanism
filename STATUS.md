...
| | |
|---|---|
| Active branch | `main` after PR #80 merged (15.D AWS Route 53 backend). Phase 15.D AWS Route 53 frontend + conformance PR in flight. |
| In-flight | **15.D AWS Route 53 frontend + SDK/CLI/Terraform conformance.** Adapter `internal/dns/frontends/aws_route53/adapter.go` implements `gen.AWSDnsV20130401Backend` translating Route 53 wire types ↔ `domain.DNS`. Wrapped error envelope (`<ErrorResponse><Error>...</Error></ErrorResponse>`) via codegen: emit.go detects `aws.protocols#restXml.noErrorWrapping`, template picks `restxml.WriteBackendErrorWrapped` (Route 53 / default) vs `WriteBackendError` (S3 — keeps the bare envelope). `services/dns/codegen.json` adds `GetChange` (terraform-aws polls it post-create); shim is synchronous so the adapter reports `INSYNC` immediately. Three conformance test files: `aws_sdk_test.go` (lifecycle + TXT double-quote round-trip), `aws_cli_test.go` (zone CRUD via `aws route53` CLI), `aws_terraform_test.go` (`hashicorp/aws` provider Apply with zone + record). Sockerless lane: `sockerless_test.go` runs through-shim (SDK → frontend → AWS backend → sockerless Route 53 sim) gated on `SOCKERLESS_AWS_ENDPOINT`; `scripts/run-sockerless-storage.sh` adds `services/dns/conformance/...`. Track A blocked on real-cloud credentials. |
