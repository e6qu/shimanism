...
| | |
|---|---|
| Active branch | `main` after PR #75 merged (15.B `_wo` investigation documented). Phase 15.B PR in flight: N10 clamp-vs-fail decision implemented. |
| In-flight | **15.B N10 decision: fail instead of clamp.** Per shimanism's "never lie / fidelity to source cloud's API" rules, silently clamping `VisibilityTimeoutSeconds` to `[10, 600]` in the GCP queue backend was the wrong default. Tightened: GCP backend now returns `domain.InvalidArgument` on values > 600 (rolling back the topic created during CreateQueue so the user doesn't see a half-created resource). The defaulting `0 → 10` stays (defaulting unset is fine; mutating user-set is not). `docs/normalizations.md` § N10 updated; open sub-question resolved. Remaining 15.B work: N13 tier-enum-vs-opaque decision, SB cross-cloud scoping. User decided to **hold off** filing the `_wo` quirk at hashicorp/aws. Track A blocked on real-cloud credentials. |
| Last merged | PR #75 — 15.B: investigate terraform-aws has_secret_string_wo drift. |
| Upstream watch | sockerless #288 merged (AWS API Gateway client-surface coverage). Zero open sockerless issues for shimanism's current work. `hashicorp/terraform-provider-aws` `has_secret_string_wo` drift documented in normalizations.md; user opted not to file upstream. |
