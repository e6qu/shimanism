...
| | |
|---|---|
| Active branch | `main` after PR #83 merged (15.D Azure DNS + Private DNS). BUG-44 (ARM passthrough) PR in flight. |
| In-flight | **BUG-44: Azure DNS ARM passthrough mode.** `internal/dns/frontends/azure_dns/server.go` adds `NewWithPassthrough(d, upstream http.Handler)`: DNS paths handled locally, every other ARM path forwarded to the configured upstream. Closes the gap that blocked Azure DNS Terraform conformance — `hashicorp/azurerm`'s single `endpoints { resource_manager = "..." }` now satisfies both DNS-specific (shim) and resource-group + subscription (sockerless ARM mock) operations. `passthrough_test.go` pins the dispatch boundary (DNS local, everything else forwarded, no-upstream returns 404). `harness.StartDNSServerAzureWithPassthrough` exposes the TLS variant + self-signed cert PEM. Through-shim Terraform test `TestSockerless_AzureDNS_Through_Shim_Terraform_Apply` wires the proxy with explicit cert pinning (RootCAs pool, no InsecureSkipVerify); SSL_CERT_FILE threads system + sockerless + shim cert bundle to Terraform. Linux-only. Track A blocked on real-cloud credentials. |
| Last merged | PR #83 — 15.D Azure DNS + Private DNS frontend + backend + SDK conformance. |
