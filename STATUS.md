...
| | |
|---|---|
| Active branch | `main` after PR #81 merged (15.D AWS Route 53 frontend + conformance). Phase 15.D GCP Cloud DNS frontend + backend + conformance PR in flight. |
| In-flight | **15.D GCP Cloud DNS frontend + backend + SDK/CLI/Terraform conformance.** Vendored `gcp-cloud-dns-discovery.json` (revision 20260520, `dns.googleapis.com`); `services/dns/gcp-codegen.json` routing-only, regen emits `services/dns/gen/gcp/gcp_clouddns.gen.go` (40 routes). `services/dns/backends/gcp/gcp.go` implements `domain.DNS` via `google.golang.org/api/dns/v1` — name→Cloud-DNS-Name derived deterministically (dots→dashes), N17 visibility dispatch, `PutRecordSet`/`DeleteRecordSet` use `Changes.Create` (the SDK-canonical atomic path), no shim-side mapping. `internal/dns/frontends/gcp_clouddns/server.go` dispatches Cloud DNS REST routes (`managedZones` + `rrsets` + `changes`) by path-shape inspection; GCP bearer verifier middleware. Three conformance tests: SDK (`google.golang.org/api/dns/v1`), CLI (`gcloud dns` via `CLOUDSDK_API_ENDPOINT_OVERRIDES_DNS`), Terraform (`hashicorp/google` `google_dns_managed_zone` + `google_dns_record_set`). **TF test runs over TLS** (`StartDNSServerGCPTLS`) because the provider's `RemoveBasePathVersion` regex matches only HTTPS — see BUG-41. Sockerless through-shim test `t.Skip`'d pending [sockerless#298](https://github.com/e6qu/sockerless/issues/298) — sim missing `Changes` API + rrset `PATCH` (BUG-42). Track A blocked on real-cloud credentials. |
