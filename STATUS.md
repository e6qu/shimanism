...
| | |
|---|---|
| Active branch | `phase-11` (PR #18 draft). Major progress: 11.0–11.3 / 11.6a–11.6d / 11.7a / 11.9a–11.10b / 11.13a all landed. 5/8 AWS frontends spec-driven and SigV4-protected (secrets, queue, functions, apigateway, storage). Remaining: awsQuery emitter (unblocks pubsub/rdbms/cache); GCS bearer + Azure Bearer/SharedKey verifiers; GCP routing emitter; Azure oapi-codegen pilot; conformance lane rewrite to sign with the test key; closer. |
| In-flight | **Phase 11 — Tighten the wire boundary.** Spec-driven codegen + signature verification (BUG-18). Landed this PR: `internal/awsjson/` runtime helper (Router, BackendError, EpochTime, QueryCompatibleCode); 3 protocol templates in the emitter (REST-XML pre-existing, awsJson1_x new, restJson1 new); generated stubs for storage (36 ops, REST-XML), secrets (11 ops, awsJson1_1), queue (14 ops, awsJson1_0), functions (14 ops, restJson1), apigateway (12 ops, restJson1); 4 AWS frontends migrated from hand-written wire to spec-driven (secrets 865 + queue 679 + lambda 493 + apigateway 490 = 2527 LOC of hand-written wire deleted); `internal/sigv4verifier/` package + middleware wired into 5/8 AWS frontends; 3 reject-path conformance tests prove enforcement end-to-end. BUG-18 downgraded P1 → P2. Honest remaining-work in [PLAN.md § Phase 11 § Remaining work](PLAN.md#remaining-work-honest). |
