...
|---|---|---|
| AWS S3 (`services/storage/backends/aws`) | Single-shot + multipart + copy round-trip | Single-shot: sockerless#173 + #174 (PR #179). Multipart (`TestSockerless_AWS_S3_Multipart`): CreateMultipartUpload → UploadPart × N (5 MiB minimum per non-final part) → ListParts → CompleteMultipartUpload → GetObject. Copy (`TestSockerless_AWS_S3_Copy`): PutObject → CopyObject (SDK `s3.CopyObject` via `x-amz-copy-source`) → GetObject, asserts dest body matches source. |
| GCS (`services/storage/backends/gcs`) | Single-shot + multipart + copy round-trip | Single-shot: SDK's `STORAGE_EMULATOR_HOST` env var. Multipart (`TestSockerless_GCS_Multipart`): Compose-based assembly. Copy (`TestSockerless_GCS_Copy`): drives the shim's `dst.CopierFrom(src).Run(ctx)` against sockerless's new `rewriteTo` REST endpoint ([sockerless PR #235](https://github.com/e6qu/sockerless/pull/235)). |
...
| Azure Service Bus queue admin (`services/queue/backends/azure`) | Admin CRUD — CreateQueue → SetQueueAttributes (LockDuration + DefaultMessageTimeToLive) → HeadQueue → ListQueues → DeleteQueue | Uses `azservicebus/admin` against sockerless's namespace-level ATOM XML admin protocol ([sockerless PR #225](https://github.com/e6qu/sockerless/pull/225)). |
| Azure Service Bus queue Send/Receive (`services/queue/backends/azure`) | Data-plane round-trip — CreateQueue → SendMessage → ReceiveMessages → DeleteQueue | Uses `azservicebus` data client against sockerless's raw AMQP/TLS listener ([sockerless PR #231](https://github.com/e6qu/sockerless/pull/231)). Integration is `azservicebus.ClientOptions.CustomEndpoint` + `TLSConfig` — same SDK-clean knob shape as the admin lane. No transport adapter code in the test layer. |
| Azure Service Bus pubsub admin (`services/pubsub/backends/azure`) | Admin CRUD — CreateTopic → CreateSubscription → ListTopics → ListSubscriptions → DeleteSubscription → DeleteTopic | Same admin protocol as the queue admin row. |
