...
| Azure Service Bus (queue + pubsub) | ⏸ Upstream blocker | The shim's `services/queue/backends/azure` + `services/pubsub/backends/azure` use the `azservicebus/admin` SDK, which speaks the **namespace-level ATOM XML admin protocol** at `<ns>.servicebus.windows.net`. Sockerless implements the ARM management API + the REST data plane, but not the ATOM XML admin protocol. Tracked at [sockerless#223](https://github.com/e6qu/sockerless/issues/223). |
